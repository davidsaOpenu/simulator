...
void INIT_SSD_CONFIG(void)
{
    ...
    while (fscanf(pfData, "%63s", key) != EOF) {
        ...
        if (key[0] == '[') {
            ...
            current_device = &devices[deviceIndex];
            deviceIndex++;
            device_count++;
...
char* GET_FILE_NAME(uint8_t deviceIndex){
	return devices[deviceIndex].file_name;
}

uint32_t GET_SECTOR_SIZE(uint8_t deviceIndex){
    return devices[deviceIndex].sector_size;
}

uint32_t GET_PAGE_SIZE(uint8_t deviceIndex){
    return devices[deviceIndex].page_size;
}
...
uint64_t GET_NAMESPACE_TOTAL_SIZE(uint8_t deviceIndex) {
    int i;
    uint64_t NAMESPACE_TOTAL_SIZE = 0;

    for (i = 0; i < MAX_NUMBER_OF_NAMESPACES; i++){
        // Check for overflow in the sum of namespaces size.
        if (UINT64_MAX - NAMESPACE_TOTAL_SIZE < devices[deviceIndex].namespaces_size[i]){
            RERR(UINT64_MAX, "ERROR, overflow detected at total size of namespaces\n");
        }

        NAMESPACE_TOTAL_SIZE += devices[deviceIndex].namespaces_size[i];
    }
