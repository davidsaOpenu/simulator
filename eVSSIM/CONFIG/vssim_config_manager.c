...
void INIT_SSD_CONFIG(void)
{
    ...
    inverse_mappings_manager = (inverse_mapping_manager_t*)malloc(sizeof(inverse_mapping_manager_t) * device_count);
    if (NULL == inverse_mappings_manager)
        RERR(, "inverse_mappings_manager allocation failed!\n");

    mapping_table = (uint64_t**)malloc(sizeof(uint64_t*) * device_count);
    if (NULL == mapping_table)
        RERR(, "mapping_table allocation failed!\n");

    g_init_ftl = (int*)malloc(sizeof(int*) * device_count);
    if (NULL == g_init_ftl)
        RERR(, "g_init_ftl allocation failed!\n");

    ssds_manager = (ssd_manager_t*)malloc(sizeof(ssd_manager_t) * device_count);
