...
void INIT_SSD_CONFIG(void)
{
    ...
    while (fscanf(pfData, "%63s", key) != EOF) {
        ...
        if (sscanf(key, "[nvme%2d]", &disk_num) == 1) {
            if (disk_num == 0 || disk_num + 1 == INVALID_DEVICE_INDEX) {
                RERR(, "Invalid device number, out of scope\n");
            }

            if (current_device == NULL && disk_num != 1)
            {
                RERR(, "Invalid device number, starting from 1\n");
            }

            if (disk_num != device_index + 1) {
                RERR(, "Invalid device number, not sequential\n");
            }
