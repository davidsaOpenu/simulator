...
void INIT_SSD_CONFIG(void)
{
    ...
    while (fscanf(pfData, "%63s", key) != EOF) {
        ...
        // Match string of format "[nvmeXX]"
        if (sscanf(key, "[ns%2d]", &current_nsid) == 1) {
            ...
            if (current_device->namespaces[current_nsid].nsid != 0) {
...
bool parse_config_line(const char* key, FILE* file, ssd_config_t* device, const uint32_t nsid) {
    ...
    if (strcmp(key, "STORAGE_STRATEGY") == 0) {
        const bool ans = fscanf(file, "%d", &device->namespaces[nsid].type) == 1;

        if (device->namespaces[nsid].type != FTL_NS_SECTOR &&
            device->namespaces[nsid].type != FTL_NS_OBJECT) {
    ...
    if (strcmp(key, "NAMESPACE_PAGE_NB") == 0) {
        ...
        return fscanf(file, "%d", &device->namespaces[nsid].ns_page_nb) == 1;
    }
    if (strcmp(key, "SIZE") == 0) {
        ...
        return fscanf(file, "%d", &device->namespaces[nsid].size) == 1;
    }
    if (strcmp(key, "OBJECT_KEY_SIZE") == 0) {
        ...
        return fscanf(file, "%d", &device->namespaces[nsid].kvkml) == 1;
    }
    if (strcmp(key, "OBJECT_MAX_VALUE_SIZE") == 0) {
        ...
        return fscanf(file, "%d", &device->namespaces[nsid].kvvml) == 1;
    }
    if (strcmp(key, "OBJECT_MAX_CAPACITY") == 0) {
        ...
        return fscanf(file, "%d", &device->namespaces[nsid].mnks) == 1;
    }
...
void calculate_derived_values(ssd_config_t* device) {
    ...
    {
        if (device->namespaces[namespaceIndex].nsid == 0){
