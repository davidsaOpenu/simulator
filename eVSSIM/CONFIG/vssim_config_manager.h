...
typedef struct {
	...
	// Only relevant while (type == FTL_NS_OBJECT).
    uint64_t size;
	uint16_t object_key_size;       // Maximum length of key in bytes (Only <=16 is supported)
	uint32_t object_max_value_size; // Maximum length of value in bytes
	uint32_t object_max_capacity;   // Maximum number of keys
} ftl_ns;
