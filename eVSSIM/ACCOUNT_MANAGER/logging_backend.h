 ...
 * Return 0 if the data were written to the logger, and non-zero otherwise
 */
int logger_write(Logger* logger, logger_data_t* buffer, int length);
...
/*
 * Read at most `length` data units from the logger to the buffer, and return the number of data units read
 * If the logger is empty, block until it is has data again
 */
int logger_read(Logger* logger, logger_data_t* buffer, int length);
