 ...
 */
void log_struct(Logger* logger, char* struct_buffer, int length, int struct_type);
...
#define LOG_REG_WRITE(logger, reg)											\
	do {																	\
		...
		log_struct(logger, (char*) &log, sizeof(log), REG_WRITE_LOG_UID);	\
	} while (0)
/*
 * Log a register read
 */
#define LOG_REG_READ(logger, reg)											\
	do {																	\
		...
		log_struct(logger, (char*) &log, sizeof(log), REG_READ_LOG_UID);	\
	} while (0)
/*...*/
