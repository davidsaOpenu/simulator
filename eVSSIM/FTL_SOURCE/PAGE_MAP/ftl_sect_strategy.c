...
ftl_ret_val _FTL_READ_SECT(uint8_t device_index, uint32_t nsid, uint64_t sector_nb, unsigned int length, unsigned char *data)
{
	...
	const uint64_t NUM_SECTORS_IN_NS = (uint64_t)NAMESPACES_SIZE[nsid] * (uint64_t)devices[device_index].sectors_per_page * (uint64_t)devices[device_index].page_nb;
	...
	io_alloc_overhead = ALLOC_IO_REQUEST(device_index, sector_nb, length, READ, &io_page_nb);
...
ftl_ret_val _FTL_WRITE_SECT(uint8_t device_index, uint32_t nsid, uint64_t sector_nb, unsigned int length, const unsigned char *data)
{
	...
	io_alloc_overhead = ALLOC_IO_REQUEST(device_index, sector_nb, length, WRITE, &io_page_nb);
	...
	while (remain > 0)
	{
		...
		if (_READ_STATUS_ENHANCED() == FTL_SUCCESS) {
			...
			ret = GET_NEW_PAGE(VICTIM_OVERALL, devices[device_index].empty_table_entry_nb, &new_ppn);
			if (ret == FTL_FAILURE) {
