...
int _FTL_READ(uint32_t sector_nb, unsigned int length)
{
	...
	while(remain > 0){
		...
		lpn = lba / (uint32_t)SECTORS_PER_PAGE;
...
int _FTL_WRITE(uint32_t sector_nb, unsigned int length)
{
	...
	if(sector_nb + length > SECTOR_NB){
		printf("Error[FTL_WRITE] Exceed Sector number: sector_nb (%d) + length (%u) (=%u) > SECTOR_NB (%ld)\n",
                sector_nb, length, sector_nb+length, SECTOR_NB);
                return FAIL;
        }
	else{
...
{
	uint32_t lpn; //The logical page address, the page that being moved.
	unsigned int ret = FAIL;
	...
	if(ret == FAIL){
#ifdef FTL_DEBUG
		printf("Error[%s] cannot copyback from %d to %d \n", __FUNCTION__, source, destination);
#endif //FTL_DEBUG
        return FAIL;
	}
