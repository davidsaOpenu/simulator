...
void INIT_MAPPING_TABLE(uint8_t device_index)
{
	...
	for (namespaceIndex = 0; namespaceIndex < MAX_NUMBER_OF_NAMESPACES; namespaceIndex++)
	{
		...
		if(fp != NULL)
		{
			if(fread(mapping_table[device_index][namespaceIndex], sizeof(uint64_t), devices[device_index].namespaces_size[namespaceIndex] * (uint64_t)devices[device_index].page_mapping_entry_nb, fp) <= 0)
				PERR("fread\n");
			fclose(fp);
		}
		else
		{
			uint64_t i;
			for(i=0; i < devices[device_index].namespaces_size[namespaceIndex] * devices[device_index].page_mapping_entry_nb; i++){
...
void TERM_MAPPING_TABLE(uint8_t device_index)
{
	...
	for (namespaceIndex = 0; namespaceIndex < MAX_NUMBER_OF_NAMESPACES; namespaceIndex++)
	{
		...
		if(fwrite(mapping_table[device_index][namespaceIndex], sizeof(uint64_t), devices[device_index].namespaces_size[namespaceIndex] * (uint64_t)devices[device_index].page_mapping_entry_nb ,fp) <= 0)
			PERR("fwrite\n");
...
{
	if(nsid >= MAX_NUMBER_OF_NAMESPACES || mapping_table[device_index][nsid] == NULL || lpn >= (devices[device_index].namespaces_size[nsid] * (uint64_t)devices[device_index].page_mapping_entry_nb)){
		PERR("overflow!\n");
	}
