...
{
	return (uint32_t)mapping_table[lpn];
}

int GET_NEW_PAGE(int mode, int mapping_index, uint32_t *ppn)
{
	empty_block_entry *curr_empty_block = GET_EMPTY_BLOCK(mode, mapping_index);
...
int UPDATE_OLD_PAGE_MAPPING(uint32_t lpn)
{
	...
	UPDATE_INVERSE_BLOCK_VALIDITY(CALC_FLASH(old_ppn), CALC_BLOCK(old_ppn), CALC_PAGE(old_ppn), PAGE_INVALID);
