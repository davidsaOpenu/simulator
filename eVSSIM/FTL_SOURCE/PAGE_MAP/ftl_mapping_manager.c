...
void INIT_MAPPING_TABLE(uint8_t device_index)
{
	...
	char* data_filename = GET_DATA_FILENAME(filename);
	...
	free(data_filename);
	if(fp != NULL){
		if(fread(mapping_table[namespaceIndex], sizeof(uint64_t), NAMESPACES_SIZE[namespaceIndex] * (uint64_t)devices[device_index].page_mapping_entry_nb, fp) <= 0)
			PERR("fread\n");
		fclose(fp);
	}
	else{
		uint64_t i;
		for(i=0; i < NAMESPACES_SIZE[namespaceIndex] * devices[device_index].page_mapping_entry_nb; i++){
...
void TERM_MAPPING_TABLE(uint8_t device_index)
{
	...
	for (namespaceIndex = 0; namespaceIndex < MAX_NUMBER_OF_NAMESPACES; namespaceIndex++)
	{
		...
		if(fwrite(mapping_table[namespaceIndex], sizeof(uint64_t), NAMESPACES_SIZE[namespaceIndex] * (uint64_t)devices[device_index].page_mapping_entry_nb ,fp) <= 0)
			PERR("fwrite\n");
...
{
	if(nsid >= MAX_NUMBER_OF_NAMESPACES || mapping_table[nsid] == NULL || lpn >= (NAMESPACES_SIZE[nsid] * (uint64_t)devices[device_index].page_mapping_entry_nb)){
		PERR("overflow!\n");
	}
...
int UPDATE_OLD_PAGE_MAPPING(uint8_t device_index,uint32_t nsid, uint64_t lpn)
{
    ...
    UPDATE_INVERSE_BLOCK_VALIDITY(CALC_FLASH(device_index, old_ppn),
...
int UPDATE_NEW_PAGE_MAPPING(uint8_t device_index, uint32_t nsid, uint64_t lpn, uint64_t ppn)
{
	...
	UPDATE_INVERSE_BLOCK_VALIDITY(CALC_FLASH(device_index, ppn), CALC_BLOCK(device_index, ppn), CALC_PAGE(device_index, ppn), PAGE_VALID);
