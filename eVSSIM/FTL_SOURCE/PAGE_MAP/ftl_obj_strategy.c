...
static obj_strategy_manager_t *obj_managers = NULL;
...
#define OBJECTS_TABLE(device_index) (obj_managers[device_index].objects_table)
#define OBJECTS_MAPPING(device_index) (obj_managers[device_index].objects_mapping)
#define GLOBAL_PAGE_TABLE(device_index) (obj_managers[device_index].global_page_table)
#define CURRENT_ID(device_index) (obj_managers[device_index].current_id)
#define OSD_DEVICE(device_index) (&obj_managers[device_index].osd)
#define OSD_SENSE(device_index) (obj_managers[device_index].osd_sense)
...
void INIT_OBJ_STRATEGY(uint8_t device_index)
{
    ...
    if (obj_managers == NULL) {
        obj_managers = calloc(device_count, sizeof(obj_strategy_manager_t));
        if (obj_managers == NULL) {
            RERR(, "Failed to allocate obj_managers array\n");
            pthread_mutex_unlock(&g_lock);
            return;
        }
    }

    if (obj_managers[device_index].initialized) {
        pthread_mutex_unlock(&g_lock);
    ...
    obj_managers[device_index].initialized = true;
    pthread_mutex_unlock(&g_lock);
}
...
void TERM_OBJ_STRATEGY(uint8_t device_index)
{
    ...
    if (obj_managers == NULL || !obj_managers[device_index].initialized) {
        pthread_mutex_unlock(&g_lock);
        return;
    }
    ...
    obj_managers[device_index].initialized = false;
    pthread_mutex_unlock(&g_lock);
}
