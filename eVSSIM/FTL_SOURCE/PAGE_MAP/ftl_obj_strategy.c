...
void INIT_OBJ_STRATEGY(uint8_t device_index)
{
	...
	for (namespaceIndex = 0; namespaceIndex < MAX_NUMBER_OF_NAMESPACES; namespaceIndex++)
	{
		...
		global_page_table[device_index][namespaceIndex] = NULL;
    }
...
void free_page_table(uint8_t device_index)
{
	...
	for (namespaceIndex = 0; namespaceIndex < MAX_NUMBER_OF_NAMESPACES; namespaceIndex++)
	{
        HASH_ITER(hh, global_page_table[device_index][namespaceIndex], current_page, tmp)
        {
            HASH_DEL(global_page_table[device_index][namespaceIndex], current_page);
            free(current_page);
        }
    }
}
...
ftl_ret_val _FTL_OBJ_WRITE(uint8_t device_index, uint32_t nsid, obj_id_t object_loc, const void *data, offset_t offset, length_t length)
{
    ...
    for (curr_io_page_nb = 0; curr_io_page_nb < io_page_nb; curr_io_page_nb++)
    {
        ...
        if (lookup_page(device_index, nsid, page_id) != NULL)
        ...
        else
        {
            ...
            HASH_DEL(global_page_table[device_index][nsid], current_page);
            current_page->page_id = page_id;
            HASH_ADD_INT(global_page_table[device_index][nsid], page_id, current_page);
        }
...
ftl_ret_val _FTL_OBJ_COPYBACK(uint8_t device_index, uint32_t nsid, int32_t source, int32_t destination)
{
    ...
    page_node *source_page = lookup_page(device_index, nsid, source);
    ...
    HASH_DEL(global_page_table[device_index][nsid], source_page);

    source_page->page_id = destination;
    HASH_ADD_INT(global_page_table[device_index][nsid], page_id, source_page);
...
int remove_object(uint8_t device_index, uint32_t nsid, stored_object *object, object_map *obj_map)
{
    ...
    while (current_page != NULL)
    {
        ...
        current_page = remove_page(device_index, nsid, current_page);
    }
...
page_node *allocate_new_page(uint8_t device_index, uint32_t nsid, object_id_t object_id, uint32_t page_id)
{
    ...
    HASH_ADD_INT(global_page_table[device_index][nsid] , page_id, new_page);
...
page_node *remove_page(uint8_t device_index, uint32_t nsid, page_node *page)
{
    ...
    // Checks if object exist in the hashtable.
    if (page->hh.tbl != NULL)
        HASH_DEL(global_page_table[device_index][nsid], page);
...
{
    page_node *page = NULL;
    HASH_FIND_INT(global_page_table[device_index][nsid], &page_id, page);
    return page;
}
