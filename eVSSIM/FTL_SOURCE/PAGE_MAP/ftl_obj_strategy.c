...
void INIT_OBJ_STRATEGY(uint8_t device_index)
{
    ...
    // Create base /tmp/osd directory if it doesn't exist
    if (stat("/tmp/osd", &st) != 0) {
        if (mkdir("/tmp/osd", 0777) != 0) {
    ...
    // Clean up device-specific directory
    snprintf(root, sizeof(root), "/tmp/osd/%d/", device_index);
    snprintf(cmd, sizeof(cmd), "rm -rf %s", root);
    if (system(cmd) != 0) {
        RERR(, "Failed to clean OSD directory for device %d\n", device_index);
        pthread_mutex_unlock(&g_lock);
        return;
    }
