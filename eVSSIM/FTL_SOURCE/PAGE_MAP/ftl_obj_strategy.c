...
void _FTL_OBJ_WRITECREATE(obj_id_t obj_loc, size_t size)
{
    ...
    if (obj_loc.create_object)
    {
        ...
        bool created = _FTL_OBJ_CREATE(obj_loc, size);
...
void OSD_WRITE_OBJ(obj_id_t obj_loc, unsigned int length, uint8_t *buf)
{
    int ret;
...
void OSD_READ_OBJ(obj_id_t obj_loc, unsigned int length, uint64_t addr, uint64_t offset)
{
    object_id_t obj_id = USEROBJECT_OID_LB + obj_loc.object_id;
    partition_id_t part_id = USEROBJECT_PID_LB + obj_loc.partition_id;
