...
void INIT_OBJ_STRATEGY(uint8_t device_index)
{
    ...
    if (obj_manager[device_index].initialized) {
        PDBG_FTL("Object strategy already initialized for device %u.\n", (unsigned int)device_index);
        return;
    }
    ...
    char root[64];
    snprintf(root, sizeof(root), OSD_PATH "%u/", (unsigned int)device_index);
...
void TERM_OBJ_STRATEGY(uint8_t device_index)
{
    ...
    char cleanup_cmd[64];
    snprintf(cleanup_cmd, sizeof(cleanup_cmd), "rm -rf " OSD_PATH "%u", (unsigned int)device_index);
    if (system(cleanup_cmd) != 0) {
        PDBG_FTL("Failed to cleanup OSD directory for device %u\n", (unsigned int)device_index);
    }
...
ftl_ret_val _FTL_OBJ_READ(uint8_t device_index, obj_id_t obj_loc, void *data, offset_t offset, length_t *p_length)
{
    ...
    for (curr_io_page_nb = 0; curr_io_page_nb < io_page_nb; curr_io_page_nb++)
    {
        ...
        ret = (ONFI_WAIT(ONFI_READ(device_index, current_page->page_id, 0, NULL, 0, &read_amount, curr_io_page_nb, READ)) == ONFI_SUCCESS) ? FTL_SUCCESS : FTL_FAILURE;
...
ftl_ret_val _FTL_OBJ_WRITE(uint8_t device_index, obj_id_t object_loc, const void *data, offset_t offset, length_t length)
{
    ...
    for (curr_io_page_nb = 0; curr_io_page_nb < io_page_nb; curr_io_page_nb++)
    {
        ...
        ret = (ONFI_WAIT(ONFI_PAGE_PROGRAM(device_index, page_id, 0, NULL, 0, &programmed_amount, curr_io_page_nb, WRITE)) == ONFI_SUCCESS) ? FTL_SUCCESS : FTL_FAILURE;
