...
// End of original vssim code without copyback
            ...
            if(ret == FTL_FAILURE){
                ...
                ONFI_WAIT(ONFI_READ(device_index, victim_phy_flash_nb * devices[device_index].pages_per_flash + victim_phy_block_nb * devices[device_index].page_nb + i, 0, NULL, 0, &read_amount, i, background ? GC_READ_BACKGROUND : GC_READ));
                ONFI_WAIT(ONFI_PAGE_PROGRAM(device_index, new_ppn, 0, NULL, 0, &programmed_amount, i, background ? GC_WRITE_BACKGROUND : GC_WRITE));
