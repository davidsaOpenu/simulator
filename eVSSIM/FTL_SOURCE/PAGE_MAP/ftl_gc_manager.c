...
static void *GC_BACKGROUND_LOOP(void *arg) {
    ...
    while (!gc_thread->gc_stop_flag) {
		...
		for (namespaceIndex = 0; namespaceIndex < MAX_NUMBER_OF_NAMESPACES; namespaceIndex++) {
			...
			do {
				collected = GC_CHECK(gc_thread->device_index, namespaceIndex, false, true);
...
bool GC_CHECK(uint8_t device_index, uint32_t nsid, bool force, bool background)
{
	int i, ret;
	...
	if(force || inverse_mappings_manager[device_index].total_zero_page_nb < devices[device_index].gc_threshold_block_nb * devices[device_index].page_nb) {
		...
		for(i=0; i<devices[device_index].gc_victim_nb; i++){
			ret = GARBAGE_COLLECTION(device_index, nsid, l2, background);
			if(ret == FTL_FAILURE){
				break;
			}
			collected = true;
...
ftl_ret_val GARBAGE_COLLECTION(uint8_t device_index, uint32_t nsid, int l2, bool background)
{
    return gc_algo.collection(device_index, nsid, l2, background);
}

ftl_ret_val DEFAULT_GC_COLLECTION_ALGO(uint8_t device_index, uint32_t nsid, int l2, bool background)
{
	int ret;
	uint64_t i;
	uint64_t lpn;
