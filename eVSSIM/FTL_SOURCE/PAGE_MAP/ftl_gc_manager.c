...
// TODO: Fix DEFAULT_GC_COLLECTION_ALGO signature
...
void GC_CHECK(uint8_t device_index, unsigned int phy_flash_nb, uint64_t phy_block_nb, bool force, bool isObjectStrategy)
{
	...
	if(force || total_empty_block_nb < (uint64_t)devices[device_index].gc_threshold_block_nb) {
        int l2 = total_empty_block_nb < (uint64_t)devices[device_index].gc_l2_threshold_block_nb;
...
// End of original vssim code without copyback
            ...
            if(ret == FTL_FAILURE){
				...
				{
            		ret = _FTL_OBJ_COPYBACK(victim_phy_flash_nb * devices[device_index].pages_per_flash + victim_phy_block_nb * devices[device_index].page_nb + i , new_ppn);
				}
