...
{
	int i, ret;
	int plane_nb = phy_block_nb % devices[current_device_index].planes_per_flash;
	uint64_t mapping_index = plane_nb * devices[current_device_index].flash_nb + phy_flash_nb;

	if(force || total_empty_block_nb < (uint64_t)devices[current_device_index].gc_threshold_block_nb) {
        int l2 = total_empty_block_nb < (uint64_t)devices[current_device_index].gc_l2_threshold_block_nb;
		for(i=0; i<devices[current_device_index].gc_victim_nb; i++){
...
ftl_ret_val DEFAULT_GC_COLLECTION_ALGO(uint64_t mapping_index, int l2, bool isObjectStrategy)
{
	...
	unsigned int victim_phy_flash_nb = devices[current_device_index].flash_nb;
...
// End of original vssim code without copyback
            ...
            if(ret == FTL_FAILURE){
                ...
                if(ret == FTL_FAILURE)
				    RERR(FTL_FAILURE, "GET_NEW_PAGE(VICTIM_OVERALL, devices[current_device_index].empty_table_entry_nb): failed\n");

                SSD_PAGE_READ(victim_phy_flash_nb, victim_phy_block_nb, i, i, GC_READ);
                SSD_PAGE_WRITE(CALC_FLASH(new_ppn), CALC_BLOCK(new_ppn), CALC_PAGE(new_ppn), i, GC_WRITE);
                old_ppn = victim_phy_flash_nb * devices[current_device_index].pages_per_flash + victim_phy_block_nb * devices[current_device_index].page_nb + i;
                lpn = GET_INVERSE_MAPPING_INFO(old_ppn);
                UPDATE_NEW_PAGE_MAPPING(lpn, new_ppn);
            }else{
            	...
            	if (!isObjectStrategy)
            		ret = _FTL_COPYBACK(victim_phy_flash_nb * devices[current_device_index].pages_per_flash + victim_phy_block_nb * devices[current_device_index].page_nb + i , new_ppn);
            	else
            		ret = _FTL_OBJ_COPYBACK(victim_phy_flash_nb * devices[current_device_index].pages_per_flash + victim_phy_block_nb * devices[current_device_index].page_nb + i , new_ppn);
...
ftl_ret_val SELECT_VICTIM_BLOCK(unsigned int* phy_flash_nb, uint64_t* phy_block_nb)
{
	...
	for (i=0;i<devices[current_device_index].victim_table_entry_nb;i++) {
	...
	if (*(victim_block->valid_page_nb) == devices[current_device_index].page_nb) {
		fail_cnt++;
		return FTL_FAILURE;
	}
