...
void INIT_INVERSE_PAGE_MAPPING(void)
{
	...
	inverse_page_mapping_table = calloc(devices[current_device_index].page_mapping_entry_nb, sizeof(uint64_t));
	if (inverse_page_mapping_table == NULL)
		RERR(, "Calloc mapping table fail\n");

	/* Initialization Inverse Page Mapping Table */
	// TODO: All the usages of GET_DATA_FILENAME are incorrect for now
	...
	if (READ_MAPPING_INFO_FROM_FILES && fp != NULL) {
		if(fread(inverse_page_mapping_table, sizeof(uint64_t), devices[current_device_index].page_mapping_entry_nb, fp) <= 0)
			PERR("fread\n");
		fclose(fp);
	}
	else{
...
void INIT_INVERSE_BLOCK_MAPPING(void)
{
	...
	inverse_block_mapping_table_start = calloc(devices[current_device_index].block_mapping_entry_nb, sizeof(inverse_block_mapping_entry));
	...
	free(filename);
	if(READ_MAPPING_INFO_FROM_FILES && fp != NULL){
		if(fread(inverse_block_mapping_table_start, sizeof(inverse_block_mapping_entry), devices[current_device_index].block_mapping_entry_nb, fp) <= 0)
			PERR("fread\n");
		fclose(fp);
	}
	else{
		...
		for(i=0;i<devices[current_device_index].block_mapping_entry_nb;i++){
...
void INIT_VALID_ARRAY(void)
{
	...
	if(READ_MAPPING_INFO_FROM_FILES && fp != NULL){
		for(i=0;i<devices[current_device_index].block_mapping_entry_nb;i++){
			valid_array = calloc(devices[current_device_index].page_nb, sizeof(char));
			if(fread(valid_array, sizeof(char), devices[current_device_index].page_nb, fp) <= 0)
				PERR("fread\n");
		...
		for(i=0;i<devices[current_device_index].block_mapping_entry_nb;i++){
			valid_array = calloc(devices[current_device_index].page_nb, sizeof(char));
			memset(valid_array,0,devices[current_device_index].page_nb);
			curr_mapping_entry->valid_array = valid_array;
...
void INIT_EMPTY_BLOCK_LIST(void)
{
	...
	empty_block_table_start = calloc(devices[current_device_index].planes_per_flash * devices[current_device_index].flash_nb, sizeof(empty_block_root));
	...
	if(READ_MAPPING_INFO_FROM_FILES && fp != NULL){
		...
		if(fread(empty_block_table_start,sizeof(empty_block_root),devices[current_device_index].planes_per_flash*devices[current_device_index].flash_nb, fp) <= 0)
			PERR("fread\n");
		curr_root = empty_block_table_start;

		for(i=0;i<devices[current_device_index].planes_per_flash;i++){

			for(j=0;j<devices[current_device_index].flash_nb;j++){
		...
		for(i=0;i<devices[current_device_index].planes_per_flash;i++){
			...
			for(j=0;j<devices[current_device_index].flash_nb;j++){
				...
				for(k=i; k < devices[current_device_index].block_nb; k+=devices[current_device_index].planes_per_flash){
				...
				curr_root->empty_block_nb = (unsigned int)devices[current_device_index].each_empty_table_entry_nb;
				curr_root += 1;
			}
		}
		total_empty_block_nb = (int64_t)devices[current_device_index].block_mapping_entry_nb;
		empty_block_table_index = 0;
	}
}
...
void INIT_VICTIM_BLOCK_LIST(void)
{
	...
	victim_block_table_start = calloc(devices[current_device_index].planes_per_flash * devices[current_device_index].flash_nb, sizeof(victim_block_root));
	...
	if(READ_MAPPING_INFO_FROM_FILES && fp != NULL){
		...
		if(fread(victim_block_table_start, sizeof(victim_block_root), devices[current_device_index].planes_per_flash*devices[current_device_index].flash_nb, fp) <= 0)
			PERR("fread\n");
...
void TERM_INVERSE_PAGE_MAPPING(void)
{
	...
	if(fwrite(inverse_page_mapping_table, sizeof(uint32_t), devices[current_device_index].page_mapping_entry_nb, fp) <= 0)
		PERR("fwrite\n");
	fclose(fp);
...
void TERM_INVERSE_BLOCK_MAPPING(void)
{
	...
	if(fwrite(inverse_block_mapping_table_start, sizeof(inverse_block_mapping_entry), devices[current_device_index].block_mapping_entry_nb, fp) <= 0)
		PERR("fwrite\n");
	fclose(fp);
...
void TERM_VALID_ARRAY(void)
{
	...
	for(i=0;i<devices[current_device_index].block_mapping_entry_nb;i++){
		...
		if(fwrite(valid_array, sizeof(char), devices[current_device_index].page_nb, fp) <= 0)
			PERR("fwrite\n");
		curr_mapping_entry += 1;
		free(valid_array);
	}
...
void TERM_EMPTY_BLOCK_LIST(void)
{
	...
	if(fwrite(empty_block_table_start,sizeof(empty_block_root),devices[current_device_index].planes_per_flash*devices[current_device_index].flash_nb, fp) <= 0)
		PERR("fwrite\n");
...
void TERM_VICTIM_BLOCK_LIST(void)
{
	...
	if(fwrite(victim_block_table_start, sizeof(victim_block_root), devices[current_device_index].planes_per_flash*devices[current_device_index].flash_nb, fp) <= 0)
		PERR("fwrite\n");
	...
	while(total_empty_block_nb != 0){
		...
		if(mode == VICTIM_OVERALL){
			...
			if(curr_root_entry->empty_block_nb == 0){
				...
				if(empty_block_table_index == devices[current_device_index].empty_table_entry_nb){
					empty_block_table_index = 0;
				}
				continue;
			}
				...
				if(curr_empty_block->curr_phy_page_nb == devices[current_device_index].page_nb){
			...
			if(curr_root_entry->empty_block_nb == 0){
				...
				mapping_index++;
				if(mapping_index % devices[current_device_index].planes_per_flash == 0){
					mapping_index = mapping_index - devices[current_device_index].planes_per_flash;
				}
			...
			if(curr_root_entry->empty_block_nb == 0){
				...
				mapping_index++;
				empty_block_table_index++;
				if (mapping_index == devices[current_device_index].empty_table_entry_nb) {
					mapping_index = 0;
					empty_block_table_index = 0;
				}
				continue;
...
ftl_ret_val INSERT_EMPTY_BLOCK(unsigned int phy_flash_nb, uint64_t phy_block_nb)
{
	...
	plane_nb = phy_block_nb % devices[current_device_index].planes_per_flash;
	mapping_index = plane_nb * devices[current_device_index].flash_nb + phy_flash_nb;
...
ftl_ret_val INSERT_VICTIM_BLOCK(empty_block_entry* full_block){
	...
	plane_nb = full_block->phy_block_nb % devices[current_device_index].planes_per_flash;
	mapping_index = plane_nb * devices[current_device_index].flash_nb + full_block->phy_flash_nb;
...
int EJECT_VICTIM_BLOCK(victim_block_entry* victim_block){
	...
	plane_nb = victim_block->phy_block_nb % devices[current_device_index].planes_per_flash;
	mapping_index = plane_nb * devices[current_device_index].flash_nb + victim_block->phy_flash_nb;
...
inverse_block_mapping_entry* GET_INVERSE_BLOCK_MAPPING_ENTRY(unsigned int phy_flash_nb, uint64_t phy_block_nb){
	...
	uint64_t mapping_index = phy_flash_nb * devices[current_device_index].block_nb + phy_block_nb;
...
int UPDATE_INVERSE_BLOCK_MAPPING(unsigned int phy_flash_nb, uint64_t phy_block_nb, int type)
{
        ...
        if(type == EMPTY_BLOCK){
                for(i=0;i<devices[current_device_index].page_nb;i++){
...
void UPDATE_VICTIM_LIST(victim_block_entry *victim_entry){
	...
	uint32_t plane_nb = victim_entry->phy_block_nb % devices[current_device_index].planes_per_flash;
	uint64_t mapping_index = plane_nb * devices[current_device_index].flash_nb + victim_entry->phy_flash_nb;
...
ftl_ret_val UPDATE_INVERSE_BLOCK_VALIDITY(unsigned int phy_flash_nb,
										  ...
										  char valid)
{
	if (phy_flash_nb >= devices[current_device_index].flash_nb || phy_block_nb >= devices[current_device_index].block_nb || phy_page_nb >= devices[current_device_index].page_nb)
