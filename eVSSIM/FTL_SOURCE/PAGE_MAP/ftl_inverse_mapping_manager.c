...
// TODO: Make this for multiple disks for all of the functions
...
void INIT_INVERSE_PAGE_MAPPING(uint8_t device_index)
{
	...
	inverse_page_mapping_table = calloc(devices[device_index].page_mapping_entry_nb, sizeof(uint64_t));
	...
	if (READ_MAPPING_INFO_FROM_FILES && fp != NULL) {
		if(fread(inverse_page_mapping_table, sizeof(uint64_t), devices[device_index].page_mapping_entry_nb, fp) <= 0)
			PERR("fread\n");
		fclose(fp);
	}
	else{
...
{
	/* Allocation Memory for Inverse Page Mapping Table */
	inverse_page_mapping_namespace_table = (void*)calloc(devices[device_index].page_mapping_entry_nb, sizeof(uint32_t));
	if (inverse_page_mapping_namespace_table == NULL)
		RERR(, "Calloc mapping table fail\n");

	/* Initialization Inverse Page Mapping Table */
	char* filename = GET_DATA_FILENAME("inverse_page_mapping_namespace.dat");
	if (filename == NULL)
		RERR(, "GET_DATA_FILENAME failed\n");
	...
	free(filename);
	if(READ_MAPPING_INFO_FROM_FILES && fp != NULL){
		if(fread(inverse_page_mapping_namespace_table, sizeof(uint32_t), devices[device_index].page_mapping_entry_nb, fp) <= 0)
			PERR("fread\n");
		fclose(fp);
	}
	else{
...
void INIT_INVERSE_BLOCK_MAPPING(uint8_t device_index)
{
	...
	inverse_block_mapping_table_start = calloc(devices[device_index].block_mapping_entry_nb, sizeof(inverse_block_mapping_entry));
	...
	free(filename);
	if(READ_MAPPING_INFO_FROM_FILES && fp != NULL){
		if(fread(inverse_block_mapping_table_start, sizeof(inverse_block_mapping_entry), devices[device_index].block_mapping_entry_nb, fp) <= 0)
			PERR("fread\n");
		fclose(fp);
	}
	else{
...
void INIT_EMPTY_BLOCK_LIST(uint8_t device_index)
{
	...
	empty_block_table_start = calloc(devices[device_index].planes_per_flash * devices[device_index].flash_nb, sizeof(empty_block_root));
	...
	if(READ_MAPPING_INFO_FROM_FILES && fp != NULL){
		...
		if(fread(empty_block_table_start,sizeof(empty_block_root),devices[device_index].planes_per_flash*devices[device_index].flash_nb, fp) <= 0)
			PERR("fread\n");
		...
		total_empty_block_nb = (int64_t)devices[device_index].block_mapping_entry_nb;
		empty_block_table_index = 0;
	}
}
...
void INIT_VICTIM_BLOCK_LIST(uint8_t device_index)
{
	...
	victim_block_table_start = calloc(devices[device_index].planes_per_flash * devices[device_index].flash_nb, sizeof(victim_block_root));
	...
	if(READ_MAPPING_INFO_FROM_FILES && fp != NULL){
		...
		if(fread(victim_block_table_start, sizeof(victim_block_root), devices[device_index].planes_per_flash*devices[device_index].flash_nb, fp) <= 0)
			PERR("fread\n");
...
void TERM_INVERSE_PAGE_MAPPING(uint8_t device_index)
{
	...
	if(fwrite(inverse_page_mapping_table, sizeof(uint64_t), devices[device_index].page_mapping_entry_nb, fp) <= 0)
		PERR("fwrite\n");
	fclose(fp);
...
void TERM_INVERSE_BLOCK_MAPPING(uint8_t device_index)
{
	...
	if(fwrite(inverse_block_mapping_table_start, sizeof(inverse_block_mapping_entry), devices[device_index].block_mapping_entry_nb, fp) <= 0)
		PERR("fwrite\n");
	fclose(fp);
...
void TERM_EMPTY_BLOCK_LIST(uint8_t device_index)
{
	...
	if(fwrite(empty_block_table_start,sizeof(empty_block_root),devices[device_index].planes_per_flash*devices[device_index].flash_nb, fp) <= 0)
		PERR("fwrite\n");
...
void TERM_VICTIM_BLOCK_LIST(uint8_t device_index)
{
	...
	if(fwrite(victim_block_table_start, sizeof(victim_block_root), devices[device_index].planes_per_flash*devices[device_index].flash_nb, fp) <= 0)
		PERR("fwrite\n");
	...
	while(total_empty_block_nb != 0){
		...
		if(mode == VICTIM_OVERALL){
			...
			if(curr_root_entry->empty_block_nb == 0){
				...
				if(empty_block_table_index == devices[device_index].empty_table_entry_nb){
					empty_block_table_index = 0;
				}
				continue;
			}
