...
Logger_Pool* logger_init(unsigned int number_of_logs) {
    ...
    for(unsigned int i=0; i<number_of_logs; i++) {
    ...
    logger_pool->number_of_allocated_logs = logger_pool->number_of_free_logs = number_of_logs;
    logger_pool->current_number_of_logs = number_of_logs;
...
int logger_write(Logger_Pool* logger_pool, Byte* buffer, int length) {
    ...
    if(logger_pool->number_of_free_logs == 0 || (LOGGER_NODE_EMPTY_BYTES(logger_pool->free_log) < length))
    {
