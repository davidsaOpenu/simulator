...
Logger* logger_init(size_t size) {
    // try to allocate the buffer (malloc is supposed to allocate memory which is aligned for
    // all primitive types)
 ...
 * If the logger can fit all the data return 0; otherwise, print a warning to stderr and return
 * nonzero
 */
