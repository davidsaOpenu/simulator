...
int offline_logger_read(Logger_Pool* logger_pool, Byte* buffer, int length){
    ...
    while(number_of_bytes_to_read > 0)
    {
        ...
        if (!(log->clean) && !(log->analyzer_done[OFFLINE_ANALYZER]))
        {
            // check how many bytes left to read in this log
            number_bytes_to_read_in_log = NUMBER_OF_BYTES_TO_READ(log, OFFLINE_ANALYZER);
            ...
            if(number_bytes_to_read_in_log >= number_of_bytes_to_read) // read length bytes from this log
            {
                ...
                if(!((NUMBER_OF_BYTES_TO_READ(log, OFFLINE_ANALYZER)-number_of_bytes_to_read) >=
                ...
                memcpy((void*) buffer, (void*) log->tails[OFFLINE_ANALYZER], number_of_bytes_to_read);
                log->tails[OFFLINE_ANALYZER] += number_of_bytes_to_read;
            ...
            { /* Tested by CrossBoundryStringWriteRead test from log_mgr_tests.cc */
                ...
                if(!((NUMBER_OF_BYTES_TO_READ(log, OFFLINE_ANALYZER)-number_bytes_to_read_in_log) >=
                            NUMBER_OF_BYTES_TO_WRITE(log)))
                {
                    // unlock logger pool
                    pthread_mutex_unlock(&logger_pool->lock);
                ...
                memcpy((void*) buffer, (void*) log->tails[OFFLINE_ANALYZER], number_bytes_to_read_in_log);
                log->tails[OFFLINE_ANALYZER] += number_bytes_to_read_in_log;
        ...
        if(NUMBER_OF_BYTES_TO_READ(log, OFFLINE_ANALYZER) == 0) {
