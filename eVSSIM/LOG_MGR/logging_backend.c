...
static int logging_writer_open_file_for_write(void) {
    uint8_t file_num = 0;
...
void logger_writer_init(void) {
    // try to create the lock
    // if (pthread_mutex_init(&logger_writer_obj.lock, NULL)) {
    //     return NULL;
    // }
    ...
    while (file_index < NUM_LOG_FILES) {
        ...
        sprintf(logger_writer_obj.log_file_path[file_index], "/tmp/log_file_%d.log", random_index);
        file_index++;
    }
    ...
    int retval = logging_writer_open_file_for_write();
...
void logger_writer_save_log_to_file(Byte *buffer, int length) {
    ...
    if (NULL != buffer) {
        fwrite(buffer, length, 1, logger_writer_obj.log_file[logger_writer_obj.curr_log_file]);
    }
    // unlock logger pool
