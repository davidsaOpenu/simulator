...
int offline_logger_read(Logger_Pool* logger_pool, Byte* buffer, int length){
    ...
    while(number_of_bytes_to_read > 0)
    {
        ...
        if (!(log->clean) && !(log->offline_analyzer_done))
        {
            // check how many bytes left to read in this log
            number_bytes_to_read_in_log = NUMBER_OF_BYTES_TO_READ_OFFLINE(log);
            ...
            if(number_bytes_to_read_in_log >= number_of_bytes_to_read) // read length bytes from this log
            {
                ...
                if(!((NUMBER_OF_BYTES_TO_READ_OFFLINE(log)-number_of_bytes_to_read) >=
            ...
            { /* Tested by CrossBoundryStringWriteRead test from log_mgr_tests.cc */
                ...
                if(!((NUMBER_OF_BYTES_TO_READ_OFFLINE(log)-number_bytes_to_read_in_log) >=
                            NUMBER_OF_BYTES_TO_WRITE(log)))
                {
                    // unlock logger pool
                    pthread_mutex_unlock(&logger_pool->lock);
        ...
        if(NUMBER_OF_BYTES_TO_READ_OFFLINE(log) == 0) {
...
int rt_logger_read(Logger_Pool* logger_pool, Byte* buffer, int length) {
    ...
    while(number_of_bytes_to_read > 0)
    {
        ...
        {
            // check how many bytes left to read in this log
            number_bytes_to_read_in_log = NUMBER_OF_BYTES_TO_READ_RT(log);

            if(number_bytes_to_read_in_log >= number_of_bytes_to_read) // read length bytes from this log
            {
                // check that we are not going to read past what has been written
                if(!((NUMBER_OF_BYTES_TO_READ_RT(log)-number_of_bytes_to_read) >=
                            NUMBER_OF_BYTES_TO_WRITE(log)))
                {
                    // unlock logger pool
                    pthread_mutex_unlock(&logger_pool->lock);
            ...
            { /* Tested by CrossBoundryStringWriteRead test from log_mgr_tests.cc */
                // check that we are not going to read past what has bean written
                if(!((NUMBER_OF_BYTES_TO_READ_RT(log)-number_bytes_to_read_in_log) >=
                            NUMBER_OF_BYTES_TO_WRITE(log)))
                {
                    // unlock logger pool
                    pthread_mutex_unlock(&logger_pool->lock);
        ...
        // check if we are done reading this log
        if(NUMBER_OF_BYTES_TO_READ_RT(log) == 0) {
