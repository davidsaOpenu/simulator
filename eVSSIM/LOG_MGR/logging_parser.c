...
static void add_metadata_to_json_object(struct json_object *jobj, const LogMetadata *m)
{
    ...
    if (m->run_id[0] != '\0') {
        ...
        // Flat _id key for filebeat (dotted field names are ambiguous in filebeat processors).
        char doc_id[RUN_ID_MAX + 32];
        snprintf(doc_id, sizeof(doc_id), "%s-%llu", m->run_id, (unsigned long long)m->seq);
        json_object_object_add(jobj, "doc_id", json_object_new_string(doc_id));
    }
