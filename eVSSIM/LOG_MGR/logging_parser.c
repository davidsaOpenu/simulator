...
void logger_busy_read(Logger_Pool* logger, Byte* buffer, int length, AnalyzerType analyzer) {
    ...
    while (bytes_read < length) {
        if(analyzer == RT_ANALYZER){
            bytes_read += rt_logger_read(logger, buffer + bytes_read, length - bytes_read);
        }
        else{
            bytes_read += offline_logger_read(logger, buffer + bytes_read, length - bytes_read);
        }
    }
}
