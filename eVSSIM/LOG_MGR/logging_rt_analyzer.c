    ...
    while (max_logs < 0 || logs_read < max_logs) {
        ...
        switch (log_type) {
            case PHYSICAL_CELL_READ_LOG_UID:
                free(NEXT_PHYSICAL_CELL_READ_LOG(analyzer->logger));
                stats.read_count++;
            ...
            case PHYSICAL_CELL_PROGRAM_LOG_UID:
                free(NEXT_PHYSICAL_CELL_PROGRAM_LOG(analyzer->logger));
                stats.write_count++;
            ...
            case LOGICAL_CELL_PROGRAM_LOG_UID:
                free(NEXT_LOGICAL_CELL_PROGRAM_LOG(analyzer->logger));
                stats.logical_write_count++;
                break;
            case GARBAGE_COLLECTION_LOG_UID:
                free(NEXT_GARBAGE_COLLECTION_LOG(analyzer->logger));
                stats.garbage_collection_count++;
                break;
            case REGISTER_READ_LOG_UID:
                free(NEXT_REGISTER_READ_LOG(analyzer->logger));
                stats.current_wall_time += REG_READ_DELAY;
                break;
            case REGISTER_WRITE_LOG_UID:
                free(NEXT_REGISTER_WRITE_LOG(analyzer->logger));
            ...
            case CHANNEL_SWITCH_TO_READ_LOG_UID:
                free(NEXT_CHANNEL_SWITCH_TO_READ_LOG(analyzer->logger));
                stats.current_wall_time += CHANNEL_SWITCH_DELAY_R;
                break;
            case CHANNEL_SWITCH_TO_WRITE_LOG_UID:
                free(NEXT_CHANNEL_SWITCH_TO_WRITE_LOG(analyzer->logger));
