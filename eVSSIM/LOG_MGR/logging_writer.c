...
static logging_writer_retval logging_writer_open_file_for_write(void) {
    ...
    writer_obj->log_file[0] = fopen(writer_obj->log_file_path[0], "w");
    if (NULL == writer_obj->log_file[0]) {
        PERR("Couldn't open file(%s) for writing!\n", writer_obj->log_file_path[0]);
        goto err_first;
    }

    writer_obj->log_file[1] = fopen(writer_obj->log_file_path[1], "w");
    if (NULL == writer_obj->log_file[1]) {
        PERR("Couldn't open file(%s) for writing!\n", writer_obj->log_file_path[1]);
        goto err_second;
    }
...
err_second:
    fclose(writer_obj->log_file[0]);
err_first:
    return retval;
}

static void logging_writer_close_file(void) {
    if (fclose(writer_obj->log_file[0]) != 0) {
        PERR("Couldn't close file(%s)\n", writer_obj->log_file_path[0]);
    }
    if (fclose(writer_obj->log_file[1]) != 0) {
        PERR("Couldn't close file(%s)\n", writer_obj->log_file_path[1]);
    }
}
...
void logging_writer_init(void) {
    ...
    sprintf(obj->log_file_path[0], "/tmp/log_file_%d_0.log", file_index); // TODO: Where should we get the file path from? ssd.conf?
    sprintf(obj->log_file_path[1], "/tmp/log_file_%d_1.log", file_index); // TODO: Where should we get the file path from? ssd.conf?
...
char *logging_writer_get_log_file_path(LoggerWriter *obj) {
    ...
    if (NULL != obj) {
        log_file_path = obj->log_file_path[obj->curr_log_file];
    }
...
void logging_writer_save_log_to_file(Log *log_obj) {
    ...
    if (buffer_size + writer_obj->curr_size > writer_obj->log_file_size) {
        ...
        writer_obj->curr_log_file = (writer_obj->curr_log_file == 0 ? 1 : 0);
    }
