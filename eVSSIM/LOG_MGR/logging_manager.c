...
extern ssd_disk ssd;
/**
 * The log manager struct (per device)
 */
...
void* log_manager_run(void* args) {
    ...
    uint8_t device_index = log_args->device_index;
    LogManager* manager = log_args->manager;
    free(log_args);
    log_manager_loop(device_index, manager, -1);
    return NULL;
}

void log_manager_loop(uint8_t device_index, LogManager* manager, int max_loops) {
    SSDStatistics old_stats = stats_init();
    // publish stats via a double buffer owned by the device: the buffers stay
    // valid after this function returns (finite max_loops), and swapping the
    // pointer atomically keeps readers from seeing a partially-copied struct
