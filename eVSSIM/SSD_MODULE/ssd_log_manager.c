...
void TERM_LOG_MANAGER(void)
{
	...
	for (i = 0; i < FLASH_NB; i++)
	    rt_log_analyzers[i]->close_flag = 1;
	log_manager->close_flag = 1;
	log_server.close_flag = 1;
	...
	free(rt_log_analyzers_threads);
	free(rt_log_analyzers);
	free(loggers);
#endif
}
