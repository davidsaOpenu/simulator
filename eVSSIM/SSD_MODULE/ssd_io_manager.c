...
int _busy_wait_until_timestamp(int64_t timestamp) {
...
//	_busy_wait_until_timestamp(wait_until);
}

// Channel access
int _ssd_channel_access(int channel) {
...
// Register write
void _ssd_reg_write(unsigned int flash_nb) {

	int channel = flash_nb % CHANNEL_NB;
	_ssd_channel_access(channel);
	if( channel_status[channel] != CHANNEL_IS_WRITE )
		_busy_wait_until_timestamp(get_usec() + CHANNEL_SWITCH_DELAY_W);
	channel_status[channel] = CHANNEL_IS_WRITE;
	channel_timer[channel] = get_usec();
}


void _ssd_cell_programming(unsigned int flash_nb, unsigned int block_nb)
{
...
ftl_ret_val SSD_PAGE_WRITE(unsigned int flash_nb, unsigned int block_nb, unsigned int page_nb, int offset, int type, int io_page_nb)
{
    ...
    // register write
	_ssd_reg_write(flash_nb);
	// wait for register write done
	_ssd_channel_access(channel);
	// cell programming
	_ssd_cell_programming(flash_nb, block_nb);
...
void _ssd_reg_read(unsigned int flash_nb) {
	...
	if( channel_status[channel] != CHANNEL_IS_READ )
		_busy_wait_until_timestamp(get_usec() + CHANNEL_SWITCH_DELAY_R);
...
void _ssd_cell_read(unsigned int flash_nb, unsigned int block_nb)
{
	_ssd_io_wait(flash_nb, block_nb);
	int plane = CALC_PLANE_BY_FLASH_BLOCK(flash_nb, block_nb);
...
ftl_ret_val SSD_PAGE_READ(unsigned int flash_nb, unsigned int block_nb, unsigned int page_nb, int offset, int type, int io_page_nb)
{
    ...
    _ssd_reg_read(flash_nb);
    _ssd_channel_access(channel);
    _ssd_cell_read(flash_nb, block_nb);
    _ssd_io_wait(flash_nb, block_nb);
...
void _ssd_block_erase(unsigned int flash_nb, unsigned int block_nb)
{
	_ssd_io_wait(flash_nb, block_nb);
...
ftl_ret_val SSD_BLOCK_ERASE(unsigned int flash_nb, unsigned int block_nb)
{
    ...
    _ssd_block_erase(flash_nb, block_nb);
    _ssd_io_wait(flash_nb, block_nb);
...
void _ssd_page_copyback(unsigned int flash_nb, unsigned int block_nb)
{
	_ssd_io_wait(flash_nb, block_nb);
	int plane = CALC_PLANE_BY_FLASH_BLOCK(flash_nb, block_nb);
	// set flash plane as on erase operation
...
ftl_ret_val SSD_PAGE_COPYBACK(uint32_t source, uint32_t destination, int type){
    ...
    _ssd_reg_read(flash_nb);
    _ssd_channel_access(channel);
    _ssd_page_copyback(flash_nb, block_nb);
    _ssd_io_wait(flash_nb, block_nb);
