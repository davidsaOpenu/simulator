...
int SSD_IO_INIT(void){
    ...
    old_channel_nb = devices[current_device_index].channel_nb;
    old_channel_time = 0;
    ...
    reg_io_cmd = (int *)malloc(sizeof(int) * devices[current_device_index].flash_nb * devices[current_device_index].planes_per_flash);
    for(i=0; i< devices[current_device_index].flash_nb*devices[current_device_index].planes_per_flash; i++){
        *(reg_io_cmd + i) = NOOP;
    }

    reg_io_type = (int *)malloc(sizeof(int) * devices[current_device_index].flash_nb * devices[current_device_index].planes_per_flash);
    for(i=0; i< devices[current_device_index].flash_nb*devices[current_device_index].planes_per_flash; i++){
        *(reg_io_type + i) = NOOP;
    }

    /* Init Register and Flash IO Time */
    reg_io_time = (int64_t *)malloc(sizeof(int64_t) * devices[current_device_index].flash_nb * devices[current_device_index].planes_per_flash);
    for(i=0; i<devices[current_device_index].flash_nb*devices[current_device_index].planes_per_flash; i++){
        *(reg_io_time +i)= -1;
    }

    cell_io_time = (int64_t *)malloc(sizeof(int64_t) * devices[current_device_index].flash_nb * devices[current_device_index].planes_per_flash);
    for(i=0; i< devices[current_device_index].flash_nb*devices[current_device_index].planes_per_flash; i++){
        *(cell_io_time + i) = -1;
    }

    /* Init Access sequence_nb */
    access_nb = (uint32_t **)malloc(sizeof(uint32_t*) * devices[current_device_index].flash_nb * devices[current_device_index].planes_per_flash);
    for(i=0; i< devices[current_device_index].flash_nb*devices[current_device_index].planes_per_flash; i++){
    ...
    /* Init IO Overhead */
    io_overhead = (int64_t *)malloc(sizeof(int64_t) * devices[current_device_index].flash_nb * devices[current_device_index].planes_per_flash);
    for(i=0; i< devices[current_device_index].flash_nb*devices[current_device_index].planes_per_flash; i++){
        *(io_overhead + i) = 0;
    }

    ssd.prev_channel_mode = (int *)malloc(sizeof(int)*devices[current_device_index].channel_nb);
    ssd.cur_channel_mode = (int *)malloc(sizeof(int)*devices[current_device_index].channel_nb);
    for (i = 0; i < devices[current_device_index].channel_nb; i++) {
...
static ftl_ret_val SSD_CELL_RECORD(int reg, int channel)
{
    ...
    switch (ssd.cur_channel_mode[channel]) {
        case WRITE:
            cell_io_time[reg] = old_channel_time + devices[current_device_index].reg_write_delay;
            break;
        case READ:
...
static int SSD_CH_RECORD(int channel, int offset, int ret)
{
    if (ssd.prev_channel_mode[channel] == READ && offset != 0 && ret == 0){
        old_channel_time += devices[current_device_index].channel_switch_delay_w;
    }
    else if (ssd.prev_channel_mode[channel] == WRITE && offset != 0 && ret == 0) {
        old_channel_time += devices[current_device_index].channel_switch_delay_r;
    }
    else {
...
ftl_ret_val SSD_PAGE_WRITE(unsigned int flash_nb, unsigned int block_nb, unsigned int page_nb, int offset, int type)
{
    ...
    reg = flash_nb*devices[current_device_index].planes_per_flash + block_nb%devices[current_device_index].planes_per_flash;

    /* Delay Operation */
    SSD_CH_ENABLE(flash_nb, channel);    // channel enable

    if( devices[current_device_index].io_parallelism == 0 ){
    ...
    if (old_channel_nb == channel && ssd.prev_channel_mode[channel] != WRITE) { //if channel is same but only mode is different
...
{
    uint32_t i;
    uint32_t r_num = flash_nb * devices[current_device_index].planes_per_flash;
    int ret = 0;

    for (i=0;i<devices[current_device_index].planes_per_flash;i++) {
...
{
    if(devices[current_device_index].channel_switch_delay_r == 0 && devices[current_device_index].channel_switch_delay_w == 0)
        return FTL_SUCCESS;
...
ftl_ret_val SSD_REG_RECORD(int reg, int type, int offset, int channel)
{
    ...
    switch (reg_io_cmd[reg]) {
        case WRITE:
            ...
            if (ssd.prev_channel_mode[channel] == READ) {
                reg_io_time[reg] += devices[current_device_index].channel_switch_delay_w;
            }
    ...
    case READ:
        ...
        if (ssd.prev_channel_mode[channel] != READ && ssd.prev_channel_mode[channel] != NOOP) {
                reg_io_time[reg] += devices[current_device_index].channel_switch_delay_r;
            }
            /* Update SATA request Info */
            if(type == READ){
                access_nb[reg][0] = io_request_seq_nb;
    ...
    case ERASE:
        ...
        break;
    case COPYBACK:
        reg_io_time[reg] = cell_io_time[reg] + devices[current_device_index].cell_read_delay;
        access_nb[reg][0] = io_request_seq_nb;
        access_nb[reg][1] = offset;
        io_update_overhead = UPDATE_IO_REQUEST(io_request_seq_nb, offset, old_channel_time, UPDATE_START_TIME);
        SSD_UPDATE_IO_OVERHEAD(reg, io_update_overhead);
...
int SSD_CH_ACCESS(unsigned int flash_nb, int channel)
{
    ...
    for (i=0;i<devices[current_device_index].way_nb;i++) {
        r_num = channel*devices[current_device_index].planes_per_flash + i*devices[current_device_index].channel_nb*devices[current_device_index].planes_per_flash;
        for(j=0;j<devices[current_device_index].planes_per_flash;j++){
...
int64_t SSD_CH_SWITCH_DELAY(unsigned int flash_nb, int channel)
{
    ...
    if (ssd.cur_channel_mode[channel] == READ ) {
        switch_delay = devices[current_device_index].channel_switch_delay_r;
    }
    else if (ssd.cur_channel_mode[channel] == WRITE) {
        switch_delay = devices[current_device_index].channel_switch_delay_w;
    }
    else{
        return 0;
    }
    ...
    switch(ssd.cur_channel_mode[channel]){
        ...
        default:{
            RERR(end - start, "New Channel mode unexpected : %d", ssd.cur_channel_mode[channel]);
            break;
        }
    }
    return end-start;
...
int SSD_REG_WRITE_DELAY(unsigned int flash_nb, int channel, int reg)
{
...
#ifdef DEL_QEMU_OVERHEAD
    if(diff < devices[current_device_index].reg_write_delay){
        SSD_UPDATE_QEMU_OVERHEAD(devices[current_device_index].reg_write_delay-diff);
    }
    ...
    int64_t delay = 0;
    if (diff < devices[current_device_index].reg_write_delay){
        wait_usec(devices[current_device_index].reg_write_delay - diff);
        delay = devices[current_device_index].reg_write_delay - diff;
        ret = 1;
    }
...
#ifdef DEL_QEMU_OVERHEAD
    if(diff < devices[current_device_index].reg_read_delay){
        SSD_UPDATE_QEMU_OVERHEAD(devices[current_device_index].reg_read_delay - diff);
    }
    diff = start - reg_io_time[reg];
#endif

    if(diff < devices[current_device_index].reg_read_delay){
        wait_usec(devices[current_device_index].reg_read_delay - diff);
        ret = 1;
    }
    end = get_usec();
...
int SSD_CELL_WRITE_DELAY(int reg)
{
...
#ifdef DEL_QEMU_OVERHEAD
    if(diff < devices[current_device_index].cell_program_delay){
        SSD_UPDATE_QEMU_OVERHEAD(devices[current_device_index].cell_program_delay-diff);
    }
    ...
    if( diff < devices[current_device_index].cell_program_delay){
        init_diff_reg = diff;
        wait_usec(devices[current_device_index].cell_program_delay - diff);
        ret = 1;
    }
...
int SSD_CELL_READ_DELAY(int reg)
{
    ...
    int64_t REG_DELAY = devices[current_device_index].cell_read_delay;
...
int SSD_BLOCK_ERASE_DELAY(int reg)
{
    ...
    if( diff < devices[current_device_index].block_erase_delay){
        wait_usec(devices[current_device_index].block_erase_delay - diff);
        ret = 1;
    }
...
int64_t SSD_GET_CH_ACCESS_TIME_FOR_READ(int channel, int reg)
{
    uint32_t i, j;
    uint32_t r_num;
    int64_t latest_time = cell_io_time[reg] + devices[current_device_index].cell_read_delay;
    ...
    for(i=0;i<devices[current_device_index].way_nb;i++){
        ...
        for(j=0;j<devices[current_device_index].planes_per_flash;j++){
            ...
            if(reg_io_cmd[r_num] == READ){
                temp_time = reg_io_time[r_num] + devices[current_device_index].reg_read_delay;
            }
            else if(reg_io_cmd[r_num] == WRITE){
                temp_time = reg_io_time[r_num] + devices[current_device_index].reg_write_delay;
            }
    ...
    for(i=0;i<devices[current_device_index].way_nb;i++){
        ...
        for(j=0;j<devices[current_device_index].planes_per_flash;j++){
            if(reg_io_cmd[r_num] == READ && reg_io_time[r_num] > current_time ){
                reg_io_time[r_num] += devices[current_device_index].reg_write_delay;
            }
            r_num++;
        }
    }
...
{
    int i;
    int p_num = devices[current_device_index].flash_nb * devices[current_device_index].planes_per_flash;
    int64_t diff = delay;
...
ftl_ret_val SSD_PAGE_COPYBACK(uint32_t source, uint32_t destination, int type){
    ...
    source_plane = CALC_FLASH(source)*devices[current_device_index].planes_per_flash + block_nb%devices[current_device_index].planes_per_flash;
    destination_plane = CALC_FLASH(destination)*devices[current_device_index].planes_per_flash + block_nb%devices[current_device_index].planes_per_flash;
...
double SSD_UTIL(void) {
    return (double)ssd.occupied_pages_counter / devices[current_device_index].pages_in_ssd;
}
