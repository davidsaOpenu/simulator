...
ftl_ret_val SSD_BLOCK_ERASE(unsigned int flash_nb, unsigned int block_nb)
{
	...
	LOG_BLOCK_ERASE(GET_LOGGER(flash_nb), (BlockEraseLog) { .channel = channel, .die = flash_nb, .block = block_nb });
...
int SSD_REG_WRITE_DELAY(unsigned int flash_nb, int channel, int reg)
{
	...
	LOG_REGISTER_WRITE(GET_LOGGER(flash_nb), (RegisterWriteLog) { .channel = channel, .die = flash_nb, .reg = reg });
...
int SSD_REG_READ_DELAY(unsigned int flash_nb, int channel, int reg)
{
	...
	LOG_REGISTER_READ(GET_LOGGER(flash_nb), (RegisterReadLog) { .channel = channel, .die = flash_nb, .reg = reg });
