...
namespace rt_subscriber {
    ...
    void write() {
        ...
        LOG_REGISTER_READ(logger, (RegisterReadLog) {
            .channel = 19, .die = 20, .reg = 21, .start_time = start, .end_time = end1
        });
        TIME_MICROSEC(end2);
        LOG_PHYSICAL_CELL_READ(logger, (PhysicalCellReadLog) {
            .channel = 1, .block = 2, .page = 3, .start_time = start, .end_time = end2
        });
        TIME_MICROSEC(end3);
        LOG_CHANNEL_SWITCH_TO_WRITE(logger, (ChannelSwitchToWriteLog) {
            .channel = 28, .start_time = start, .end_time = end3
        });
        TIME_MICROSEC(end4);
        LOG_PHYSICAL_CELL_PROGRAM(logger, (PhysicalCellProgramLog) {
            .channel = 4, .block = 5, .page = 6, .start_time = start, .end_time = end4
        });
        TIME_MICROSEC(end5);
        LOG_GARBAGE_COLLECTION(logger, (GarbageCollectionLog) empty_log);
        LOG_LOGICAL_CELL_PROGRAM(logger, (LogicalCellProgramLog) {
            .channel = 7, .block = 8, .page = 9, .start_time = start, .end_time = end5
        });
        TIME_MICROSEC(end6);
        LOG_REGISTER_WRITE(logger, (RegisterWriteLog) {
            .channel = 22, .die = 23, .reg = 24, .start_time = start, .end_time = end6
        });
        TIME_MICROSEC(end7);
        LOG_PHYSICAL_CELL_PROGRAM(logger, (PhysicalCellProgramLog) {
            .channel = 10, .block = 11, .page = 12, .start_time = start, .end_time = end7
        });
        TIME_MICROSEC(end8);
        LOG_LOGICAL_CELL_PROGRAM(logger, (LogicalCellProgramLog) {
            .channel = 13, .block = 14, .page = 15, .start_time = start, .end_time = end8
        });
        TIME_MICROSEC(end9);
        LOG_BLOCK_ERASE(logger, (BlockEraseLog) {
            .channel = 25, .die = 26, .block = 27, .start_time = start, .end_time = end9
        });
        TIME_MICROSEC(end10);
        LOG_CHANNEL_SWITCH_TO_READ(logger, (ChannelSwitchToReadLog) {
            .channel = 29, .start_time = start, .end_time = end10
        });
        TIME_MICROSEC(end11);
        LOG_PHYSICAL_CELL_READ(logger, (PhysicalCellReadLog) {
            .channel = 16, .block = 17, .page = 18, .start_time = start, .end_time = end11
        });
        LOG_GARBAGE_COLLECTION(logger, (GarbageCollectionLog) empty_log);
    }
