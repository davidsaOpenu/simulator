...
valgrind --leak-check=full --error-exitcode=2 ./simulation_tests_main | tee simulation_tests_main.log || status=$?
valgrind --leak-check=full --error-exitcode=2 ./unit_tests_main | tee unit_tests_main.log || status=$?
