...
void MONITOR_STATS_UPDATE(int expected_duration) {
    ...
    for( i = 0; i < FLASH_NB; i++ ) {
        ...
        AnalyzerHandler handler = log_manager->handlers[i];
        SSDStatistics flash_rt_log_stats = handler.slots[handler.current_slot];
...
namespace {
    ...
    TEST_P(SSDIoEmulatorUnitTest, BWCase2) {
        ...
        int logical_page_reads = 0;
        int channel_read_changes = 0;
        ...
        for(i = 0; i < FLASH_NB; i++) {
            for(j = 0; j <BLOCK_NB; j++) {
                for(k = 0; k < PAGE_NB; k++) {
                    ...
                    logical_page_writes++;
                    usleep(REG_WRITE_DELAY + CELL_PROGRAM_DELAY);
                }
            }
        }

        for(i = 0; i < FLASH_NB; i++) {
            channel_read_changes++;
            for(j = 0; j < BLOCK_NB; j++) {
                for(k = 0; k < PAGE_NB; k++) {
        ...
        // single write page delay
        int expected_write_duration = (REG_WRITE_DELAY + CELL_PROGRAM_DELAY) * logical_page_writes;
        int expected_read_duration = CHANNEL_SWITCH_DELAY_R * channel_read_changes
                + (REG_READ_DELAY + CELL_READ_DELAY) * logical_page_reads;
...
//        ASSERT_LE(abs((int)total_read_delay - expected_read_duration), DELAY_THRESHOLD);
     ...
     * - execute iterate write on all pages in disk
     * - after any page write verify QT monitor ssd utilization equals \ssd_utils
     * - after any page write verify new monitor ssd utilization equals \ssd_utils
     */
    TEST_P(SSDIoEmulatorUnitTest, SSDUtilCase7) {
        ...
        double ssd_utils = 0;
        int expected_write_duration = REG_WRITE_DELAY + CELL_PROGRAM_DELAY;
        int expected_block_erase_duration = BLOCK_ERASE_DELAY;
        ...
        for( i = 0; i < FLASH_NB; i++ ) {
            for( j = 0; j < BLOCK_NB; j++ ) {
                ...
                ssd_utils = (double)occupied_pages / PAGES_IN_SSD;
                MONITOR_STATS_UPDATE(expected_block_erase_duration);
                ASSERT_EQ(ssd_utils, SSD_UTIL());
            }
        }
    }
