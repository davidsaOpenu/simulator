...
//	}
	...
	TEST_P(SSDIoEmulatorUnitTest, WriteBWAndDurationOnSingleWrite) {
		...
		ASSERT_GE(GET_IO_BANDWIDTH((double)exp_duration / 1), SSD_WRITE_BANDWIDTH());
		ASSERT_LE(GET_IO_BANDWIDTH((double)(exp_duration + 3) / 1), SSD_WRITE_BANDWIDTH());
	}

	/**
	 * Write bandwidth
	 * on sequential page write
	 * without io overhead
	 */
	TEST_P(SSDIoEmulatorUnitTest, WriteBWAndDurationOnTwoSequentialWrite) {
	 ...
	 */
	TEST_P(SSDIoEmulatorUnitTest, BWOnSwitchSequentialWriteCase1) {
		...
		ASSERT_GE(GET_IO_BANDWIDTH((double)exp_write_duration / 2), SSD_WRITE_BANDWIDTH());
		ASSERT_LE(GET_IO_BANDWIDTH((double)(exp_write_duration + 8) / 2), SSD_WRITE_BANDWIDTH());

		ASSERT_GE(GET_IO_BANDWIDTH((double)exp_read_duration / 1), SSD_READ_BANDWIDTH());
		ASSERT_LE(GET_IO_BANDWIDTH((double)(exp_read_duration + 8) / 1), SSD_READ_BANDWIDTH());
	}

	/**
	 * Write bandwidth
	 * on two parallel page writes
	 * with io overhead on register & cell programming
	 */
	TEST_P(SSDIoEmulatorUnitTest, BWOnTwoParallelWriteRequestsCase1) {
		...
		usleep(500000);
		ASSERT_GE(GET_IO_BANDWIDTH((double)exp_duration / 2), SSD_WRITE_BANDWIDTH());
	 ...
	 */
	TEST_P(SSDIoEmulatorUnitTest, BWOnTwoParallelWriteRequestsCase2) {
	...
	/**
	 * Write bandwidth
	 * on two parallel page write on two different channels
	 * without io overhead
	 */
	TEST_P(SSDIoEmulatorUnitTest, BWOnTwoParallelWriteRequestsCase3) {
		...
		//		1st cell programming done
		exp_duration += CELL_PROGRAM_DELAY;
		// time=@REG_WRITE_DELAY + @CELL_PROGRAM_DELAY + @usec_diff
		//		2nd cell programming done
		exp_duration += CELL_PROGRAM_DELAY;
		...
		// at time: @(current_timestamp + 1000)
		run_ssd_page_write_thread(current_timestamp + 1000
				, flash_nb_1,0,0,0, WRITE, -1);
		// run SSD_PAGE_WRITE(0,0,1,0, WRITE, -1) method
		// at time: @(current_timestamp + 1000 + usec_diff)
		run_ssd_page_write_thread(current_timestamp + 1000 + usec_diff
				, flash_nb_2,0,0,0, WRITE, -1);
