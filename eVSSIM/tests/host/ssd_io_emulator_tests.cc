...
namespace {
    ...
    /**
     * @brief testing delay caused by full disk write & read
     * - write all disk pages
     * - read all disk pages
     * - verify QT monitor total write bandwidth delay is \expected_write_duration
     * - TODO: verify Browser monitor total write bandwidth delay is \expected_write_duration
     */
    TEST_P(SSDIoEmulatorUnitTest, BWCase2) {

        int offset = 0;
        int logical_page_writes = FLASH_NB * BLOCK_NB * PAGE_NB;
        int logical_page_reads = FLASH_NB * BLOCK_NB * PAGE_NB;
        int channel_read_changes = FLASH_NB;
