...
namespace {
    ...
    std::vector<SSDConf> GetParams() {
        ...
        test_params.push_back(SSDConf(parameters::Allsizemb[0], DEFAULT_FLASH_NB));
    ...
    TEST_P(SSDIoEmulatorUnitTest, WriteRead1) {
        ...
        int flash_nb = 0;
        int block_nb = 0;
        int expected_write_duration = (REG_WRITE_DELAY + CELL_PROGRAM_DELAY) * test_params.get_pages_per_block();
        int expected_read_duration = CHANNEL_SWITCH_DELAY_R + (REG_READ_DELAY + CELL_READ_DELAY) * test_params.get_pages_per_block();
        int expected_rw = test_params.get_pages_per_block();

        // Write all pages in the block
        for (size_t i = 0; i < test_params.get_pages_per_block(); i++) {
    ...
    TEST_P(SSDIoEmulatorUnitTest, WriteRead2) {
        ...
        size_t block_x_flash = test_params.get_pages() / test_params.get_pages_per_block();
        size_t blocks_per_flash = block_x_flash / flash_num;

        int expected_rw = test_params.get_pages_per_block() * blocks_per_flash;
        ...
        for (size_t i = 0; i < blocks_per_flash; i++) {
            for (size_t j = 0; j < test_params.get_pages_per_block(); j++) {
    ...
    TEST_P(SSDIoEmulatorUnitTest, WriteAmplificationTest) {
        ...
        for(int x=0; x<2; x++){
            for(size_t p=0; p < test_params.get_pages(); p++){
                ASSERT_EQ(FTL_SUCCESS, _FTL_WRITE_SECT(p * test_params.get_page_size(), 1));
            }
        }
