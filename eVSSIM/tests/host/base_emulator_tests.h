...
namespace {
    ...
    public:
        ...
        BaseEmulatorParams(size_t size_mb, size_t flash_nb) {
            size_t pages_= size_mb * ((1024 * 1024) / CONST_PAGE_SIZE_IN_BYTES); // number_of_pages = disk_size (in MB) * 1048576 / page_size
        ...
        BaseEmulatorParams(size_t page_size, size_t page_nb, size_t sector_size, size_t flash_nb, size_t block_nb, size_t channel_nb) {
    ...
    ostream& operator<<(ostream& os, const BaseEmulatorParams& value) {
        (void) value;
        return os;
    }

    class BaseEmulatorTests : public BaseEmulatorParams, public ::testing::TestWithParam<BaseEmulatorParams> {
        public:

            virtual void SetUp(BaseEmulatorParams test_params) {
