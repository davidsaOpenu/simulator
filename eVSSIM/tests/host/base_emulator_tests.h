...
namespace {
    ...
    private:
        ...
        void ssd_conf_calc_based_size_mb(size_t size_mb) {
            this->pages = size_mb * ((1024 * 1024) / CONST_PAGE_SIZE_IN_BYTES); // number_of_pages = disk_size (in MB) * 1048576 / page_size
            size_t block_x_flash = this->pages / CONST_PAGES_PER_BLOCK; // all_blocks_on_all_flashes = number_of_pages / pages_in_block
