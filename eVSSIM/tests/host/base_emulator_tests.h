...
namespace {
    ...
    public:
// One constr that initializes the disk size
// The  other one that calcualtes other small  things

        SSDConf(size_t size_mb) {
            // move the calculations out of constructor
            // move to a dedicated function that will apply calculation per test needs
            // play a little with the calculation, pay attention to 1024 * 1024
            ...
            size_t blocks_per_flash = block_x_flash / DEFAULT_FLASH_NB; // number_of_flashes = all_blocks_on_all_flashes / number_of_blocks_in_flash
