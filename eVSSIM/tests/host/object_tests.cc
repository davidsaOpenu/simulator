...
//                                                                        6 * 1024 * 1024)); // 6 MB
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreate) {
        ...
        unique_object_id object_locator = { .object_id = 0, .partition_id = 0 };
#ifndef NO_OSD
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreateWrite) {
        ...
        unique_object_id objects[objects_in_ssd_];
        ...
        for(unsigned long p=1; p < objects_in_ssd_/2; p++){
            ASSERT_EQ(FTL_SUCCESS, _FTL_OBJ_WRITE(&objects[p],(uint8_t *)wrbuf,0,GET_PAGE_SIZE()));
#ifndef NO_OSD
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreateRead) {
        ...
        for(unsigned long p=1; p < objects_in_ssd_/2; p++){
            ASSERT_EQ(FTL_SUCCESS, _FTL_OBJ_READ(&objects[p],(uint8_t *)rdbuf, 0,GET_PAGE_SIZE()));
#ifndef NO_OSD
    ...
    TEST_P(ObjectUnitTest, ObjectGrowthTest) {
        ...
        unique_object_id tempObj = { .object_id = 1000, .partition_id = 0 };
        ...
        while (size < final_object_size) {
            ASSERT_EQ(FTL_SUCCESS, _FTL_OBJ_WRITE(&tempObj,(uint8_t *)wrbuf, size, object_size_));
            size += object_size_;
#ifndef NO_OSD
