...
namespace write_read_test{
    ...
    TEST_P(WriteReadTest, WRITEREADWRITETest){
        ...
        //when GC is invoked, We expect at most one block copy back, if src block and dest block are not on the same flash, a r/w is issued instead of a copyback

        //when all reallocations are whithin the same flash, copyback always works
        uint64_t r_w_count_best_case = total_pages;
        //when every write triggers a GC reallocation and all reallocations are between different flashs.
