...
namespace {
    ...
    class DelayCalcUnitTest : public ::testing::TestWithParam<std::pair<size_t,size_t> > {
        public:
    ...
    INSTANTIATE_TEST_CASE_P(DiskSize, DelayCalcUnitTest, ::testing::ValuesIn(GetParams()));
    // Check if single SSD page write takes
    // more than REG_WRITE_DELAY + CELL_PROGRAM_DELAY
	TEST_P(DelayCalcUnitTest, WriteDelayGraterThenRegisterPageAndProgramDelay) {
		...
		ASSERT_GE(duration, REG_WRITE_DELAY + CELL_PROGRAM_DELAY);
	}
