...
namespace log_mgr_tests {
    ...
    TEST_P(LogMgrUnitTest, ReadBeforeWriteOnLogBoundry) {
        ...
        for (unsigned int i = 0; i < ((LOG_SIZE)-1); i++)
            ASSERT_EQ(1, logger_read(_logger, (Byte *)res, 1, RT_ANALYZER));
        ASSERT_EQ(0, logger_read(_logger, (Byte *)res, 5 ,RT_ANALYZER));
    }
    /**
    ...
    TEST_P(LogMgrUnitTest, CleanOfflineAnalyzerNotDone)
    {
        ...
        // read full log
        for (unsigned long int i = 0; i < (LOG_SIZE * _logger->number_of_allocated_logs); i++) {
            ASSERT_EQ(1, logger_read(_logger, (Byte*)read_res, 1, RT_ANALYZER));
        }
    ...
    TEST_P(LogMgrUnitTest, FullPoolStringWriteRead) {
        ...
        // read full log
        for (unsigned long int i = 0; i < (LOG_SIZE * _logger->number_of_allocated_logs); i++) {
            ASSERT_EQ(1, logger_read(_logger, &placeholder, 1, RT_ANALYZER));
        }
    ...
    TEST_P(LogMgrUnitTest, SuccessiveWrites) {
        ...
        ASSERT_EQ(1, logger_read(_logger, &first_res, 1, RT_ANALYZER));
        ASSERT_EQ(1, logger_read(_logger, &second_res, 1, RT_ANALYZER));
    ...
    TEST_P(LogMgrUnitTest, ReadBeforeWrite) {
        Byte placeholder = 'J';
        Byte res;
        ASSERT_EQ(0, logger_read(_logger, &res, 1, RT_ANALYZER));
        ASSERT_EQ(0, logger_write(_logger, &placeholder, RT_ANALYZER));
        ASSERT_EQ(1, logger_read(_logger, &res, 1, RT_ANALYZER));
    ...
    TEST_P(LogMgrUnitTest, GarbageCollection) {
        ...
        Byte placeholder;
        ASSERT_EQ(0, logger_read(_logger, &placeholder, 0, RT_ANALYZER));
