...
namespace log_mgr_tests {
    ...
    /**
     * Test read before write test case 1
     */
    TEST_P(LogMgrUnitTest, ReadBeforeWrite1) {
        Byte placeholder = 'x';
        char res[5];

        // second case of read with out write
    ...
    /**
     * Test read before write test case 2
     */
    TEST_P(LogMgrUnitTest, ReadBeforeWrite2) {
        Byte placeholder = 'J';
        Byte res;

        // write 1 byte to make the log-clean = false
        ASSERT_EQ(0, logger_write(_logger, &placeholder, 1));
        // read 1 byte
        ASSERT_EQ(1, logger_read(_logger, &res, 1));
        // try to read 1 more byte that should lead
        // to reading 0 bytes because nothing has been
        // written
    ...
    TEST_P(LogMgrUnitTest, NormalStringFillWriteRead) {
        ...
        for (unsigned int i = 0;
                i < (unsigned int)((LOGGER_BUFFER_POOL_SIZE * _logger->number_of_allocated_logs)/sizeof(TEST_STRING));
                i++){
            ASSERT_EQ(0, logger_write(_logger, (Byte*) TEST_STRING, sizeof(TEST_STRING)));
        }
    ...
    TEST_P(LogMgrUnitTest, FullBuffer) {
        ...
        for (unsigned int i = 0; i < ((LOGGER_BUFFER_POOL_SIZE * _logger->number_of_allocated_logs)); i++)
            ASSERT_EQ(0, logger_write(_logger, &placeholder, 1));
    ...
    TEST_P(LogMgrUnitTest, FullPoolStringWriteRead) {
        // fill the buffer
        Byte placeholder = 'x';
        for (unsigned long int i = 0; i < ((LOGGER_BUFFER_POOL_SIZE * _logger->number_of_allocated_logs)); i++) {
            ASSERT_EQ(0, logger_write(_logger, &placeholder, 1));
        }
        // write test string
        ASSERT_EQ(0, logger_write(_logger, (Byte*) TEST_STRING, sizeof(TEST_STRING)));
    ...
    TEST_P(LogMgrUnitTest, WrapStringWriteRead) {
        // fill the buffer
        Byte placeholder = 'x';
        for (unsigned int i = 0; i < LOGGER_BUFFER_POOL_SIZE - (sizeof(TEST_STRING)); i++) {
            ASSERT_EQ(0, logger_write(_logger, &placeholder, 1));
        }
