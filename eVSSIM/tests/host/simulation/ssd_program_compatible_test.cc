...
namespace program_compatible_test
{
    ...
    TEST_P(ProgramCompatibleTest, ReadAfterWriteAsProgramCompatibleToAllSDDTest)
    {
        ...
        ASSERT_EQ(ssd_config->get_pages_ns(ID_SECTOR_NS0), log_server.stats.read_count);
        ASSERT_EQ(2 * ssd_config->get_pages_ns(ID_SECTOR_NS0), log_server.stats.write_count); // We write twice to every page but as program compatible (only 1 -> 0)
    ...
    TEST_P(ProgramCompatibleTest, ReadAfterWriteNotAsProgramCompatibleToAllSDDTest)
    {
        ...
        // checks that log_server.stats (the stats on the monitor) are accurate
        ASSERT_LT(2 * ssd_config->get_pages_ns(ID_SECTOR_NS0), log_server.stats.write_count); // We write twice to every page but not as program compatible.
