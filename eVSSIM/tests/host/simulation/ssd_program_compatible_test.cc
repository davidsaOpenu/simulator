...
namespace program_compatible_test
{
    ...
    TEST_P(ProgramCompatibleTest, SimpleProgramCompatibleOnOnePageTest)
    {
        ...
        ASSERT_EQ(_FTL_WRITE(g_device_index, ID_NS0, 0, sizeof(data) / ssd_config->get_sector_size(), data), FTL_SUCCESS);

        unsigned char read_data[sizeof(data)];
        ASSERT_EQ(_FTL_READ(g_device_index, ID_NS0, 0, sizeof(read_data) / ssd_config->get_sector_size(), read_data), FTL_SUCCESS);
    ...
    TEST_P(ProgramCompatibleTest, WriteProgramCompatibleMultiplePagesSingleFtlCall)
    {
        ...
        memset(data, 0x0F, sizeof(data));
        ASSERT_EQ(_FTL_WRITE(g_device_index, ID_NS0, offset / ssd_config->get_sector_size(), sizeof(data) / ssd_config->get_sector_size(), data), FTL_SUCCESS);

        ASSERT_EQ(_FTL_READ(g_device_index, ID_NS0, offset / ssd_config->get_sector_size(), sizeof(read_data) / ssd_config->get_sector_size(), read_data), FTL_SUCCESS);
        ASSERT_EQ(memcmp(data, read_data, sizeof(data)), 0);
    ...
    TEST_P(ProgramCompatibleTest, ReadAfterWriteAsProgramCompatibleToAllSDDTest)
    {
        ...
        uint64_t check_trigger = (double)ssd_config->get_pages_ns(ID_NS0) * CHECK_THRESHOLD;
        ...
        for (unsigned int p = 0; p < ssd_config->get_pages_ns(ID_NS0); p++)
        {
            // Write data twice to the page
            memset(data, 0x0F, sizeof(data));
            ASSERT_EQ(_FTL_WRITE(g_device_index, ID_NS0, p * ssd_config->get_page_size() / ssd_config->get_sector_size(),
        ...
        for (unsigned int p = 0; p < ssd_config->get_pages_ns(ID_NS0); p++)
        {
            ...
            ASSERT_EQ(_FTL_READ(g_device_index, ID_NS0, p * ssd_config->get_page_size() / ssd_config->get_sector_size(),
        ...
        ASSERT_EQ(ssd_config->get_pages_ns(ID_NS0), log_server.stats.read_count);
        ASSERT_EQ(2 * ssd_config->get_pages_ns(ID_NS0), log_server.stats.write_count); // We write twice to every page but as program compatible (only 1 -> 0)
    ...
    TEST_P(ProgramCompatibleTest, ReadAfterWriteNotAsProgramCompatibleToAllSDDTest)
    {
        ...
        // checks that log_server.stats (the stats on the monitor) are accurate
        ASSERT_LT(2 * ssd_config->get_pages_ns(ID_NS0), log_server.stats.write_count); // We write twice to every page but not as program compatible.
