...
namespace program_compatible_test
{
    ...
    TEST_P(ProgramCompatibleTest, SimpleProgramCompatibleOnOnePageTest)
    {
        ...
        ASSERT_EQ(_FTL_WRITE(g_device_index, ID_SECTOR_NS0, 0, sizeof(data) / ssd_config->get_sector_size(), data), FTL_SUCCESS);

        unsigned char read_data[sizeof(data)];
        ASSERT_EQ(_FTL_READ(g_device_index, ID_SECTOR_NS0, 0, sizeof(read_data) / ssd_config->get_sector_size(), read_data), FTL_SUCCESS);
    ...
    TEST_P(ProgramCompatibleTest, WriteProgramCompatibleMultiplePagesSingleFtlCall)
    {
        ...
        memset(data, 0x0F, sizeof(data));
        ASSERT_EQ(_FTL_WRITE(g_device_index, ID_SECTOR_NS0, offset / ssd_config->get_sector_size(), sizeof(data) / ssd_config->get_sector_size(), data), FTL_SUCCESS);

        ASSERT_EQ(_FTL_READ(g_device_index, ID_SECTOR_NS0, offset / ssd_config->get_sector_size(), sizeof(read_data) / ssd_config->get_sector_size(), read_data), FTL_SUCCESS);
        ASSERT_EQ(memcmp(data, read_data, sizeof(data)), 0);
    ...
    TEST_P(ProgramCompatibleTest, ReadAfterWriteAsProgramCompatibleToAllSDDTest)
    {
        ...
        uint64_t check_trigger = (double)ssd_config->get_pages_ns(ID_SECTOR_NS0) * CHECK_THRESHOLD;
        ...
        for (unsigned int p = 0; p < ssd_config->get_pages_ns(ID_SECTOR_NS0); p++)
        {
            ...
            memset(data, 0x0F, sizeof(data));
            ASSERT_EQ(_FTL_WRITE(g_device_index, ID_SECTOR_NS0, p * ssd_config->get_page_size() / ssd_config->get_sector_size(),
        ...
        for (unsigned int p = 0; p < ssd_config->get_pages_ns(ID_SECTOR_NS0); p++)
        {
            ...
            ASSERT_EQ(_FTL_READ(g_device_index, ID_SECTOR_NS0, p * ssd_config->get_page_size() / ssd_config->get_sector_size(),
        ...
        ASSERT_EQ(ssd_config->get_pages_ns(ID_SECTOR_NS0), log_server.stats[g_device_index].read_count);
        ASSERT_EQ(2 * ssd_config->get_pages_ns(ID_SECTOR_NS0), log_server.stats[g_device_index].write_count); // We write twice to every page but as program compatible (only 1 -> 0)
    ...
    TEST_P(ProgramCompatibleTest, ReadAfterWriteNotAsProgramCompatibleToAllSDDTest)
    {
        ...
        for (unsigned int p = 0; p < ssd_config->get_pages_ns(ID_SECTOR_NS0); p++)
        {
            ...
            if (action_count >= check_trigger)
            {
                ...
                expected_stats.read_count = log_server.stats[g_device_index].write_count - expected_stats.write_count;
                ASSERT_EQ(expected_stats.read_count, log_server.stats[g_device_index].read_count);
                ASSERT_EQ(expected_stats.logical_write_count, log_server.stats[g_device_index].logical_write_count);
        ...
        // checks that log_server.stats (the stats on the monitor) are accurate
        ASSERT_LT(2 * ssd_config->get_pages_ns(ID_SECTOR_NS0), log_server.stats[g_device_index].write_count); // We write twice to every page but not as program compatible.
        ASSERT_LT(0, log_server.stats[g_device_index].garbage_collection_count); // Because the write we do is NOT program compatible GC should be called
    }
} // namespace
