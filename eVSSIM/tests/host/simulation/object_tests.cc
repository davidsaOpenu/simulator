...
namespace object_tests {
    class ObjectUnitTest : public BaseTest {
        public:
            virtual void SetUp() {
                ...
                int object_pages = (int)ceil(1.0 * object_size_ / GET_PAGE_SIZE(g_device_id)); // ceil because we can't have a page belong to 2 objects
                objects_in_ssd_ = (unsigned int)((devices[g_device_id].pages_in_ssd - devices[g_device_id].page_nb) / object_pages); //over-provisioning of exactly one block
            }
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreate) {
        ...
        printf("Page no.:%ld\nPage size:%d\n", devices[g_device_id].pages_in_ssd, GET_PAGE_SIZE(g_device_id));
        ...
        for (unsigned long p = 0; p < objects_in_ssd_; p++) {
            ...
            bool res = _FTL_OBJ_CREATE(g_device_id, object_locator, object_size_);
            ASSERT_TRUE(res);
        }
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreateWrite) {
        ...
        for (unsigned long p = 1; p < objects_in_ssd_ / 2; p++) {
            ...
            bool res = _FTL_OBJ_CREATE(g_device_id, objects[p], object_size_);
            ASSERT_TRUE(res);
        }

        char *wrbuf = (char *)Calloc(1, GET_PAGE_SIZE(g_device_id));

        // Write GET_PAGE_SIZE(g_device_id) data to each one
        for (unsigned long p = 1; p < objects_in_ssd_ / 2; p++) {
            ASSERT_EQ(FTL_SUCCESS, _FTL_OBJ_WRITE(g_device_id, objects[p], wrbuf, 0, GET_PAGE_SIZE(g_device_id)));
        }
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreateWriteRead) {
        ...
        for(unsigned long p= 1; p < objects_in_ssd_ / 2; p++) {
            ...
            res = _FTL_OBJ_WRITE(g_device_id, objects[p], wrbuf, 0, object_size_);
        ...
        uint32_t len = 0;
        char *rdbuf = (char *)Calloc(1, GET_PAGE_SIZE(g_device_id) / 2);

        // Read GET_PAGE_SIZE(g_device_id) / 2 data from each one
        for(unsigned long p = 1; p < objects_in_ssd_ / 2; p++) {
            len = GET_PAGE_SIZE(g_device_id) / 2;
            memset(rdbuf, 0x0, GET_PAGE_SIZE(g_device_id) / 2);
            ASSERT_EQ(FTL_SUCCESS, _FTL_OBJ_READ(g_device_id, objects[p], rdbuf, 0, &len));
            sprintf(wrbuf, "%lu", objects[p].object_id);
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreateWriteDelete) {
        ...
        for (unsigned long p = 1; p < objects_in_ssd_ / 2; p++) {
            ...
            ASSERT_EQ(FTL_SUCCESS, _FTL_OBJ_DELETE(g_device_id, objects[p]));
        }
