...
namespace object_tests {
    ...
    class ObjectUnitTest : public BaseTest {
        public:
            virtual void SetUp() {
                ...
                objects_in_default_ns = (unsigned int)(((ssd_config->get_pages_ns(ID_OBJECT_NS0) - ssd_config->get_page_nb()) * ssd_config->get_page_size()) / object_size);
            }
    ...
    TEST_P(ObjectUnitTest, InvalideNmaespaceStrategy) {
        obj_id_t object_locator = { .object_id = 0, .partition_id = USEROBJECT_PID_LB };

        ASSERT_FALSE(FTL_OBJ_CREATE(g_device_index, ID_SECTOR_NS0, object_locator, object_size));
        ASSERT_FALSE(FTL_OBJ_CREATE(g_device_index, ID_SECTOR_NS1, object_locator, object_size));
    }
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreate) {
        ...
        for (unsigned long p = 0; p < this->objects_in_default_ns; p++) {
            ...
            ASSERT_TRUE(FTL_OBJ_CREATE(g_device_index, ID_OBJECT_NS0, object_locator, object_size));
        }

        // At this step there shouldn't be any free page.
        ASSERT_FALSE(FTL_OBJ_CREATE(g_device_index, ID_OBJECT_NS0, object_locator, object_size));

        // The other namespace isn't full.
        ASSERT_TRUE(FTL_OBJ_CREATE(g_device_index, ID_OBJECT_NS1, object_locator, object_size));
    }
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreateWrite) {
        ...
        ASSERT_EQ(FTL_FAILURE, FTL_OBJ_WRITE(g_device_index, ID_OBJECT_NS0, object_locator, wrbuf, 0, GET_PAGE_SIZE(g_device_index)));

        // Create a new object and write to it.
        ASSERT_TRUE(FTL_OBJ_CREATE(g_device_index, ID_OBJECT_NS0, object_locator, object_size));
        ASSERT_EQ(FTL_SUCCESS, FTL_OBJ_WRITE(g_device_index, ID_OBJECT_NS0, object_locator, wrbuf, 0, GET_PAGE_SIZE(g_device_index)));

        // Try to write into an object in different namespace
        ASSERT_EQ(FTL_FAILURE, FTL_OBJ_WRITE(g_device_index, ID_OBJECT_NS1, object_locator, wrbuf, 0, GET_PAGE_SIZE(g_device_index)));
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreateWriteRead) {
        ...
        for (unsigned long p = 1; p < this->objects_in_default_ns / 2; p++) {
            ...
            ASSERT_EQ(FTL_SUCCESS, FTL_OBJ_WRITE(g_device_index, ID_OBJECT_NS0, object_locator, wrbuf, 0, object_size));
        }
        ...
        for(unsigned long p = 1; p < this->objects_in_default_ns / 2; p++) {
            ...
            ASSERT_EQ(FTL_SUCCESS, FTL_OBJ_READ(g_device_index, ID_OBJECT_NS0, object_locator, rdbuf, 0, &len));
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreateWriteDelete) {
        ...
        for (unsigned long p = 1; p < this->objects_in_default_ns / 2; p++) {
            ...
            ASSERT_EQ(FTL_SUCCESS, FTL_OBJ_DELETE(g_device_index, ID_OBJECT_NS0, object_locator));
    ...
    TEST_P(ObjectUnitTest, ObjectGrowthTest) {
        ...
        ASSERT_TRUE(FTL_OBJ_CREATE(g_device_index, ID_OBJECT_NS0, tempObj, object_size));
        ...
        while (offset < final_object_size) {
            ASSERT_EQ(FTL_SUCCESS, FTL_OBJ_WRITE(g_device_index, ID_OBJECT_NS0, tempObj, wrbuf, 0, object_size));
            offset += object_size;
        }
    ...
    TEST_P(ObjectUnitTest, ObjectInvalidNamespace) {
        ...
        ASSERT_FALSE(FTL_OBJ_CREATE(g_device_index, ID_OBJECT_NS0 + 10, object_locator, object_size));
        ...
        uint32_t len = 0;
        ASSERT_EQ(FTL_FAILURE, FTL_OBJ_READ(g_device_index, ID_OBJECT_NS0 + 10, object_locator, buff, 0, &len));
        ASSERT_EQ(FTL_FAILURE, FTL_OBJ_WRITE(g_device_index, ID_OBJECT_NS0 + 10, object_locator, buff, 0, object_size));
