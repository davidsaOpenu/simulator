...
namespace object_tests {
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreate) {
        ...
        for (unsigned long p = 0; p < this->objects_in_default_ns; p++) {
            ...
            ASSERT_TRUE(FTL_OBJ_CREATE(g_device_index, object_locator, object_size));
        }

        // At this step there shouldn't be any free page.
        ASSERT_FALSE(FTL_OBJ_CREATE(g_device_index, object_locator, object_size));
    }
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreateWrite) {
        ...
        ASSERT_EQ(FTL_FAILURE, FTL_OBJ_WRITE(g_device_index, object_locator, wrbuf, 0, GET_PAGE_SIZE(g_device_index)));

        // Create a new object and write to it.
        ASSERT_TRUE(FTL_OBJ_CREATE(g_device_index, object_locator, object_size));
        ASSERT_EQ(FTL_SUCCESS, FTL_OBJ_WRITE(g_device_index, object_locator, wrbuf, 0, GET_PAGE_SIZE(g_device_index)));
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreateWriteRead) {
        ...
        for (unsigned long p = 1; p < this->objects_in_default_ns / 2; p++) {
            ...
            ASSERT_EQ(FTL_SUCCESS, FTL_OBJ_WRITE(g_device_index, object_locator, wrbuf, 0, object_size));
        }
        ...
        for(unsigned long p = 1; p < this->objects_in_default_ns / 2; p++) {
            ...
            ASSERT_EQ(FTL_SUCCESS, FTL_OBJ_READ(g_device_index, object_locator, rdbuf, 0, &len));
    ...
    TEST_P(ObjectUnitTest, SimpleObjectCreateWriteDelete) {
        ...
        for (unsigned long p = 1; p < this->objects_in_default_ns / 2; p++) {
            ...
            ASSERT_EQ(FTL_SUCCESS, FTL_OBJ_DELETE(g_device_index, object_locator));
    ...
    TEST_P(ObjectUnitTest, ObjectGrowthTest) {
        ...
        ASSERT_TRUE(FTL_OBJ_CREATE(g_device_index, tempObj, object_size));
        ...
        while (offset < final_object_size) {
            ASSERT_EQ(FTL_SUCCESS, FTL_OBJ_WRITE(g_device_index, tempObj, wrbuf, 0, object_size));
            offset += object_size;
        }
