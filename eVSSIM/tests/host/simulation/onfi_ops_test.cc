...
namespace program_compatible_test
{
    ...
    // SSD_REG_RECORD runs once per register operation, so anything it logs lands on the
    // hot path: a single unconditional line there once buried a run of PageProgramAllSuccess
    // under ~1.7M lines and stalled CI. Assert logging does not scale with I/O count.
    TEST_P(OnfiCommandsTest, PageProgramDoesNotLogPerOperation)
    {
        ...
        EXPECT_LT(captured_lines, pages_to_program)
            << "logged " << captured_lines << " lines for "
