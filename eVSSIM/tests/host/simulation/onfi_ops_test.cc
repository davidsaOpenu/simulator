...
namespace program_compatible_test
{
    ...
    TEST_P(OnfiCommandsTest, StatusRegisterFailValuesCorrectAfterBlockEraseOperation)
    {
        ...
        ASSERT_EQ(ONFI_BLOCK_ERASE(g_device_index, ssd_config->get_pages_ns(DEFAULT_NSID)), ONFI_FAILURE);
        ASSERT_EQ(ONFI_READ_STATUS(g_device_index, &status), ONFI_SUCCESS);
        ASSERT_EQ(status.FAIL, 1);
        ASSERT_EQ(status.FAILC, 0);
    ...
    TEST_P(OnfiCommandsTest, OutOfBoundsRowAddressReadFails)
    {
        ...
        ASSERT_EQ(ONFI_READ(g_device_index, ssd_config->get_pages_ns(DEFAULT_NSID), 0, buffer, ssd_config->get_page_size(), &nread), ONFI_FAILURE);
    }
    ...
    TEST_P(OnfiCommandsTest, OutOfBoundsRowAddressPageProgramFails)
    {
        ...
        ASSERT_EQ(ONFI_PAGE_PROGRAM(g_device_index, ssd_config->get_pages_ns(DEFAULT_NSID), 0, buffer, ssd_config->get_page_size(), &nprogrammed), ONFI_FAILURE);
    }
