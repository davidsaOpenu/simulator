...
#define ID_SECTOR_NS0 0
#define ID_SECTOR_NS1 1
#define ID_OBJECT_NS0 2
#define ID_OBJECT_NS1 3
...
namespace {
    ...
    private:
        ...
        int ns_strategies[MAX_NUMBER_OF_NAMESPACES] = {0,};
        ...
        void ssd_conf_calc_based_size_mb(size_t size_mb) {
            ...
            memset(block_ns_nb, 0, sizeof(block_ns_nb));

            // Set the number of blocks per namespace.
            // Sector namespace 0 owns the full physical flash so that writing
            // all of its pages achieves 80 % device utilisation (25 % OP).
            // Sector namespace 1 is kept at zero because the public FTL API no
            // longer exposes a namespace selector (nsid is hard-coded to 0).
            block_ns_nb[ID_SECTOR_NS0] = block_x_flash;
            block_ns_nb[ID_SECTOR_NS1] = 0;
            block_ns_nb[ID_OBJECT_NS0] = (block_x_flash / 2);
            block_ns_nb[ID_OBJECT_NS1] = (block_x_flash / 2);
        }
    ...
    public:
        SSDConf(size_t size_mb, size_t sector_size = 1) {
            ...
            ns_strategies[ID_SECTOR_NS0] = FTL_NS_SECTOR;
            ns_strategies[ID_SECTOR_NS1] = FTL_NS_SECTOR;
            ns_strategies[ID_OBJECT_NS0] = FTL_NS_OBJECT;
            ns_strategies[ID_OBJECT_NS1] = FTL_NS_OBJECT;
        }
        ...
        SSDConf(size_t page_size, size_t page_nb, size_t sector_size,
                ...
                : page_size(page_size), page_nb(page_nb), sector_size(sector_size),
                  flash_nb(flash_nb), block_nb(block_nb), channel_nb(channel_nb) {
                    ...
                    block_ns_nb[ID_SECTOR_NS0] = default_ns_block_nb;
                    block_ns_nb[ID_SECTOR_NS1] = othere_ns_block_nb;
                    block_ns_nb[ID_OBJECT_NS0] = default_ns_block_nb;
                    block_ns_nb[ID_OBJECT_NS1] = othere_ns_block_nb;
        ...
        size_t get_device_pages_ns() {
            ...
            {
                page_sum += get_total_pages_ns(nsid);
            }
        ...
        int get_ns_strategy(uint32_t nsid) {
            return ns_strategies[nsid];
        }

        uint32_t get_num_namespaces() {
        ...
        void ssd_conf_serialize(void) {
            ...
            ssd_conf << "[nvme01]\n"
                ...
                "NAMESPACE_PAGE_NB " << get_pages_ns(ID_SECTOR_NS0) << "\n"
                "[ns02]\n"
                "STORAGE_STRATEGY " << FTL_NS_SECTOR << "\n"
                "NAMESPACE_PAGE_NB " << get_pages_ns(ID_SECTOR_NS1) << "\n"
                "[ns03]\n"
                "STORAGE_STRATEGY " << FTL_NS_OBJECT << "\n"
                "SIZE " << get_pages_ns(ID_OBJECT_NS0) * get_page_size() << "\n"
                "OBJECT_KEY_SIZE " << get_object_size() << "\n"
                "OBJECT_MAX_VALUE_SIZE 4096\n"
                "OBJECT_MAX_CAPACITY 4096\n"
                "[ns04]\n"
                "STORAGE_STRATEGY " << FTL_NS_OBJECT << "\n"
                "SIZE " << get_pages_ns(ID_OBJECT_NS1) * get_page_size() << "\n"
                "OBJECT_KEY_SIZE " << get_object_size() << "\n"
                "OBJECT_MAX_VALUE_SIZE 4096\n"
                "OBJECT_MAX_CAPACITY 4096\n"
                "[nvme02]\n"
                ...
                "[ns01]\n"
                "STORAGE_STRATEGY " << FTL_NS_OBJECT << "\n"
                "SIZE " << get_pages_ns(ID_SECTOR_NS0) * get_page_size() << "\n"
                "OBJECT_KEY_SIZE " << get_object_size() << "\n"
                "OBJECT_MAX_VALUE_SIZE 4096\n"
                "OBJECT_MAX_CAPACITY 4096\n"
                "[ns02]\n"
                "STORAGE_STRATEGY " << FTL_NS_OBJECT << "\n"
                "SIZE " << get_pages_ns(ID_SECTOR_NS1) * get_page_size() << "\n"
                "OBJECT_KEY_SIZE " << get_object_size() << "\n"
                "OBJECT_MAX_VALUE_SIZE 4096\n"
                "OBJECT_MAX_CAPACITY 4096\n"
                "[nvme03]\n"
                ...
                "[ns01]\n"
                "STORAGE_STRATEGY " << FTL_NS_SECTOR << "\n"
                "NAMESPACE_PAGE_NB " << get_total_pages_ns(ID_SECTOR_NS0) << "\n"                "[ns02]\n"
                "STORAGE_STRATEGY " << FTL_NS_OBJECT << "\n"
                "SIZE 4096\n"
                "OBJECT_KEY_SIZE " << get_object_size() << "\n"
                "OBJECT_MAX_VALUE_SIZE 4096\n"
                ...
                "STAT_TYPE 15\n"
                "STAT_SCOPE 62\n"
                "STAT_PATH /tmp/stat4.csv\n"
                "GC_LOW_THR 20\n"
                "GC_HI_THR 80\n"
                "[ns01]\n"
                "STORAGE_STRATEGY " << FTL_NS_SECTOR << "\n"
                "NAMESPACE_PAGE_NB " << get_total_pages_ns(ID_SECTOR_NS0) + get_total_pages_ns(ID_SECTOR_NS1) << "\n"
                "[nvme05]\n"
                ...
                "STAT_TYPE 15\n"
                "STAT_SCOPE 62\n"
                "STAT_PATH /tmp/stat5.csv\n"
                "GC_LOW_THR 20\n"
                "GC_HI_THR 80\n"
                "[ns01]\n"
                "STORAGE_STRATEGY " << FTL_NS_SECTOR << "\n"
                "NAMESPACE_PAGE_NB 100\n"
                "[ns02]\n"
                "STORAGE_STRATEGY " << FTL_NS_SECTOR << "\n"
                "NAMESPACE_PAGE_NB 100\n"
                "[ns03]\n"
    ...
    class BaseTest : public ::testing::TestWithParam<SSDConf*> {
        ...
        public:
            virtual void SetUp(void) {
                ...
                {
                    pages_in_ssd += ssd_config->get_total_pages_ns(namespaceIndex);
                }
