...
namespace ssd_io_emulator_tests {
    ...
    TEST_P(GCTest, CaseWaitForInvalidPage) {
        ...
        for (uint64_t page_counter = 0; page_counter < base_test_get_ssd_config()->get_total_pages_ns(ID_SECTOR_NS0); page_counter++) {
        ...
        for (uint64_t page_counter = 0; page_counter < base_test_get_ssd_config()->get_total_pages_ns(ID_SECTOR_NS1) - config->page_nb; page_counter++) {
            uint64_t lpn = page_counter;
            ASSERT_EQ(FTL_SUCCESS, GET_NEW_PAGE(g_device_index, VICTIM_OVERALL, devices[g_device_index].empty_table_entry_nb, &ppn));
    ...
    TEST_P(GCTest, CaseReservedLBAs) {
        ...
        const uint64_t sectors_in_first_namespace = config->sectors_per_page * (base_test_get_ssd_config()->get_total_pages_ns(ID_SECTOR_NS0));
        const uint64_t sectors_in_second_namespace = config->sectors_per_page * (base_test_get_ssd_config()->get_total_pages_ns(ID_SECTOR_NS1));
    ...
    TEST_P(GCTest, CasePerfGCThreadDisable) {
        ...
        EXPECT_LT(1.1, log_server.stats[g_device_index].write_amplification);
    }
