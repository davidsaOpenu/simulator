...
namespace ssd_io_emulator_tests {
    ...
    std::vector<SSDConf*> GetTestParams() {
        ...
        ssd_configs.push_back(new SSDConf(parameters::sizemb::mb1, true));
    ...
    TEST_P(GCTest, CaseReservedLBAs) {
        ...
        const uint64_t sectors_in_defult_namespace = config->sectors_per_page * (base_test_get_ssd_config()->get_total_pages_ns(DEFAULT_NSID) - config->page_nb);
