...
namespace ssd_io_emulator_tests {
    ...
    TEST_P(GCTest, CaseWaitForInvalidPage) {
        ssd_config_t *config = &devices[g_device_index];
        // With 80% OP the disk has free pages above gc_hi_thr_page_nb after fill.
        // Without this, the GC would spin via timedwait(0) instead of blocking.
        config->gc_hi_thr_interval_sec = 100 * 3600; // 100 hours — practically infinity
        ...
        // Fill NS0 completely.  The remaining zero pages are the 20 % OP pages.
        ...
        // With 20 % OP the over-provisioned pages keep VICTIM_OVERALL succeeding; the fill
    ...
    TEST_P(GCTest, CaseDiskFullWrite) {
        ssd_config_t *config = &devices[g_device_index];
        // With 20 % OP the disk has free pages above gc_hi_thr_page_nb after fill.
        // Without this, the GC would spin via timedwait(0) instead of blocking.
        config->gc_hi_thr_interval_sec = 100 * 3600; // 100 hours — practically infinity
        ...
        // The fill above leaves zero_after_fill free pages (the OP pool), which is
        // well above the GC threshold.
        ASSERT_EQ(zero_after_fill, inverse_mappings_manager[g_device_index].total_zero_page_nb);
        ...
        // several arbitrary sectors within NS0:
        ...
        // new zero consumed); with 20% OP the disk never hits the device_full threshold, so
        ...
        for (size_t i = 0; i < sizeof(sectorsFirstNamespace) / sizeof(sectorsFirstNamespace[0]); i++) {
            ...
            pthread_mutex_unlock(&g_lock);
            ASSERT_EQ(FTL_SUCCESS, FTL_WRITE_SECT(g_device_index, sectorsFirstNamespace[i], 1, NULL));
            pthread_mutex_lock(&g_lock);
    ...
    TEST_P(GCTest, CaseReservedLBAs) {
        ...
        ASSERT_EQ(FTL_FAILURE, FTL_WRITE_SECT(g_device_index, sectors_in_first_namespace, 1, NULL));
        ASSERT_EQ(FTL_SUCCESS, FTL_WRITE_SECT(g_device_index, sectors_in_first_namespace - 1, 1, NULL));
        pthread_mutex_lock(&g_lock);
    }
