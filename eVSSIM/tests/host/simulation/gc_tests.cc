...
namespace ssd_io_emulator_tests {
    ...
    TEST_P(GCTest, CaseBackgroundGC) {
        ...
        for (uint64_t i = 0; i < config->pages_in_ssd / 2; i++) {
            ...
            ASSERT_EQ(FTL_SUCCESS, UPDATE_NEW_PAGE_MAPPING(g_device_index, ID_SECTOR_NS0, lpn, ppn));
            ASSERT_EQ(FTL_SUCCESS, UPDATE_OLD_PAGE_MAPPING(g_device_index, ID_SECTOR_NS0, lpn));
        }
    ...
    TEST_P(GCTest, CaseWaitForInvalidPage) {
        ssd_config_t *config = &devices[g_device_index];
        // With 80% OP the disk has 74 free pages after fill, which is above gc_hi_thr_page_nb
        // (64 for mb1).  Without this, the GC would spin via timedwait(0) instead of blocking.
        config->gc_hi_thr_interval_sec = 100 * 3600; // 100 hours — practically infinity
        ...
        // With 80% over-provisioning, NAMESPACE_PAGE_NB = get_pages_ns() per namespace.
        // Total filled = get_pages_ns(NS0) + (get_pages_ns(NS1) - page_nb).
        // Remaining zero pages = pages_in_ssd - total_filled (always > page_nb due to OP).
        const uint64_t zero_after_fill = config->pages_in_ssd
            - base_test_get_ssd_config()->get_pages_ns(ID_SECTOR_NS0)
            - (base_test_get_ssd_config()->get_pages_ns(ID_SECTOR_NS1) - config->page_nb);

        for (uint64_t page_counter = 0; page_counter < base_test_get_ssd_config()->get_pages_ns(ID_SECTOR_NS0); page_counter++) {
            uint64_t lpn = page_counter;
            ASSERT_EQ(FTL_SUCCESS, GET_NEW_PAGE(g_device_index, VICTIM_OVERALL, devices[g_device_index].empty_table_entry_nb, &ppn));
            ASSERT_EQ(FTL_SUCCESS, UPDATE_NEW_PAGE_MAPPING(g_device_index, ID_SECTOR_NS0, lpn, ppn));
        }

        for (uint64_t page_counter = 0; page_counter < base_test_get_ssd_config()->get_pages_ns(ID_SECTOR_NS1) - config->page_nb; page_counter++) {
            uint64_t lpn = page_counter;
            ASSERT_EQ(FTL_SUCCESS, GET_NEW_PAGE(g_device_index, VICTIM_OVERALL, devices[g_device_index].empty_table_entry_nb, &ppn));
            ASSERT_EQ(FTL_SUCCESS, UPDATE_NEW_PAGE_MAPPING(g_device_index, ID_SECTOR_NS1, lpn, ppn));
        }

        // With 80% OP the over-provisioned pages keep VICTIM_OVERALL succeeding; the fill
        // above leaves zero_after_fill free pages, which is well above the GC threshold.

        ASSERT_EQ(0, gc_threads[g_device_index].gc_loop_count);
        ASSERT_EQ(zero_after_fill, inverse_mappings_manager[g_device_index].total_zero_page_nb);
        ...
        for (uint64_t page_counter = 0; page_counter < base_test_get_ssd_config()->get_pages_ns(ID_SECTOR_NS1) - config->page_nb; page_counter++) {
            uint64_t lpn = page_counter;
            ASSERT_EQ(FTL_SUCCESS, UPDATE_OLD_PAGE_MAPPING(g_device_index, ID_SECTOR_NS1, lpn));
        }
        ...
        ASSERT_LT(zero_after_fill, inverse_mappings_manager[g_device_index].total_zero_page_nb);
    }
    ...
    TEST_P(GCTest, CaseDiskFullWrite) {
        ...
        const uint64_t sectors_in_first_namespace = config->sectors_per_page * (base_test_get_ssd_config()->get_pages_ns(ID_SECTOR_NS0));
        const uint64_t sectors_in_second_namespace = config->sectors_per_page * (base_test_get_ssd_config()->get_pages_ns(ID_SECTOR_NS1));

        // several arbitrary sectors:
        uint64_t sectorsFirstNamespace[] = {
            0, 1, sectors_in_first_namespace / 2 - 1, sectors_in_first_namespace / 2,sectors_in_first_namespace / 2 + 1,
            sectors_in_first_namespace - 2, sectors_in_first_namespace - 1};

        uint64_t sectorsSecondNamespace[] = {
            0, 1, sectors_in_second_namespace / 2 - 1, sectors_in_second_namespace / 2,sectors_in_second_namespace / 2 + 1,
            sectors_in_second_namespace - 2, sectors_in_second_namespace - 1};

        // "sparse surface scan" — each write overwrites an existing page (valid → invalid +
        // new zero consumed); with 80% OP the disk never hits the device_full threshold, so
        // each write uses VICTIM_OVERALL normally and zero_page_nb decreases by one per write.
        for (size_t i = 0; i < sizeof(sectorsFirstNamespace) / sizeof(sectorsFirstNamespace[0]); i++) {
            ASSERT_GE(inverse_mappings_manager[g_device_index].total_zero_page_nb, config->page_nb);

            pthread_mutex_unlock(&g_lock);
            ASSERT_EQ(FTL_SUCCESS, FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS0, sectorsFirstNamespace[i], 1, NULL));
            ASSERT_EQ(FTL_SUCCESS, FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS1, sectorsSecondNamespace[i], 1, NULL));
            pthread_mutex_lock(&g_lock);
    ...
    TEST_P(GCTest, CaseReservedLBAs) {
        ...
        ASSERT_EQ(FTL_FAILURE, FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS0, sectors_in_first_namespace, 1, NULL));
        ASSERT_EQ(FTL_SUCCESS, FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS0, sectors_in_first_namespace - 1, 1, NULL));
        pthread_mutex_lock(&g_lock);

        pthread_mutex_unlock(&g_lock);
        // "Exceed Sector number" error - reserved pages for GC
        ASSERT_EQ(FTL_FAILURE, FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS1, sectors_in_second_namespace, 1, NULL));
        ASSERT_EQ(FTL_SUCCESS, FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS1, sectors_in_second_namespace - 1, 1, NULL));
        pthread_mutex_lock(&g_lock);
    }
    ...
    TEST_P(GCTest, CasePerfGCThreadDisable) {
        ...
        const uint64_t max_sector_nb = config->sectors_per_page * base_test_get_ssd_config()->get_pages_ns(ID_SECTOR_NS0);

        unsigned int seed = 0;
        for (uint64_t i = 0; i < 10 * config->pages_in_ssd; i++) {
            uint64_t sector_nb = rand_r(&seed) % max_sector_nb;
            ASSERT_EQ(FTL_SUCCESS, _FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS0, sector_nb, 1, NULL));
        }
    ...
    TEST_P(GCTest, CasePerfGCThreadEnable) {
        ...
        for (uint64_t i = 0; i < 10 * config->pages_in_ssd; i++) {
            uint64_t sector_nb = rand_r(&seed) % max_sector_nb;
            ASSERT_EQ(FTL_SUCCESS, FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS0, sector_nb, 1, NULL));
