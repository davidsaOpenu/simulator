...
namespace manager_subscriber {
    ...
    void on_update(SSDStatistics stats, void*) {
        ...
        // in the second update check for the right values
            ...
            unsigned int time_per_write = devices[g_device_index].cell_program_delay;

            double write_speed = CALCULATEMBPS((num_of_logical_writes * GET_PAGE_SIZE(g_device_index)), (num_of_phy_writes * time_per_write));

            unsigned int time_per_read = devices[g_device_index].cell_read_delay;
            double read_speed = CALCULATEMBPS(GET_PAGE_SIZE(g_device_index), time_per_read);
    ...
    void run() {
        ...
        log_manager_loop(g_device_index, manager, 1);
        ...
        rt_log_analyzer_loop(g_device_index, analyzer2, 3);
