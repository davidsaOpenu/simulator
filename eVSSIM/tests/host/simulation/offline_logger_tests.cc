...
void flipAuto();
    ...
    std::unordered_map<std::string, int> predict_test_stats(int mode) {
        ...
        switch(mode) {
            case MODE_R:
                stats["PhysicalCellReadLog"] = devices[g_device_id].pages_in_ssd;
                stats["RegisterReadLog"] = devices[g_device_id].pages_in_ssd;
                stats["ChannelSwitchToReadLog"] = devices[g_device_id].flash_nb;
                break;
            case MODE_W:
                stats["RegisterWriteLog"] = devices[g_device_id].pages_in_ssd;
                stats["LogicalCellProgramLog"] = devices[g_device_id].pages_in_ssd;
                stats["PhysicalCellProgramLog"] = devices[g_device_id].pages_in_ssd;
                stats["ChannelSwitchToWriteLog"] = devices[g_device_id].flash_nb;
                break;
            case MODE_RW:
                stats["RegisterWriteLog"] = devices[g_device_id].pages_in_ssd;
                stats["RegisterReadLog"] = devices[g_device_id].pages_in_ssd;
                ...
                stats["PhysicalCellProgramLog"] = devices[g_device_id].pages_in_ssd;
                stats["PhysicalCellReadLog"] = devices[g_device_id].pages_in_ssd;
                stats["ChannelSwitchToReadLog"] = devices[g_device_id].pages_in_ssd;
                stats["ChannelSwitchToWriteLog"] = devices[g_device_id].pages_in_ssd;
                break;
        }
        return stats;
    }
    ...
    void readOrWrite(int mode){
        ...
        int expected_write_duration = (
            devices[g_device_id].channel_switch_delay_w +
            devices[g_device_id].reg_write_delay +
            devices[g_device_id].cell_program_delay
        ) * devices[g_device_id].pages_in_ssd;

        int expected_read_duration = (
            devices[g_device_id].channel_switch_delay_r +
            devices[g_device_id].reg_read_delay +
            devices[g_device_id].cell_read_delay
        ) * devices[g_device_id].pages_in_ssd;

        // In case of r/w mode: read / write num_blocks

        for (uint64_t i = 0; i < devices[g_device_id].pages_in_ssd; i++) {
            switch (mode) {
                case MODE_R:
                    SSD_PAGE_READ(g_device_id, CALC_FLASH(g_device_id, i), CALC_BLOCK(g_device_id, i), i, 0, READ);
                    break;
                case MODE_W:
                    SSD_PAGE_WRITE(g_device_id, CALC_FLASH(g_device_id, i), CALC_BLOCK(g_device_id, i), i, 0, WRITE);
                    break;
                case MODE_RW:
                    SSD_PAGE_READ(g_device_id, CALC_FLASH(g_device_id, i), CALC_BLOCK(g_device_id, i), i, 0, READ);
                    SSD_PAGE_WRITE(g_device_id, CALC_FLASH(g_device_id, i), CALC_BLOCK(g_device_id, i), i, 0, WRITE);
