...
namespace write_read_test
{
        ...
        for (uint32_t nsid = 0; nsid < ssd_config->get_num_namespaces(); nsid++)
        {
            ...
            if (totalPages > 0 && ssd_config->get_ns_strategy(nsid) == FTL_NS_SECTOR)
            {
                ...
                for (size_t p = 0; p < totalPages; p++)
                {
                    ...
                    ASSERT_EQ(FTL_SUCCESS, FTL_WRITE_SECT(g_device_index, p * pageSize, pageSize, write_buffer));
                }

                for (size_t p = 0; p < totalPages; p++)
                {
                    memset(write_buffer, (uint8_t)(p % 255), pageSize);
                    ASSERT_EQ(FTL_SUCCESS, FTL_READ_SECT(g_device_index, p * pageSize, pageSize, read_buffer));
    ...
    TEST_P(WriteReadTest, BoundaryViolationTest)
    {
        SSDConf *ssd_config = base_test_get_ssd_config();
        size_t pageSize = ssd_config->get_page_size();
        size_t totalPages = ssd_config->get_pages_ns(ID_SECTOR_NS0);
        ...
        ASSERT_EQ(FTL_FAILURE, FTL_WRITE_SECT(g_device_index, invalid_offset, 1, buffer));
        ASSERT_EQ(FTL_FAILURE, FTL_READ_SECT(g_device_index, invalid_offset, 1, buffer));
    ...
    void ns_stress_worker(int device_idx, uint32_t nsid, size_t total_pages, size_t page_size)
    {
        ...
        for (int loop = 0; loop < 3; loop++)
        {
            for (size_t p = 0; p < total_pages; p++)
            {
                ...
                ASSERT_EQ(FTL_SUCCESS, FTL_WRITE_SECT(device_idx, p * page_size, page_size, write_buffer));
            }

            for (size_t p = 0; p < total_pages; p++)
            {
                memset(write_buffer, (uint8_t)((p + nsid + loop) % 255), page_size);
                ASSERT_EQ(FTL_SUCCESS, FTL_READ_SECT(device_idx, p * page_size, page_size, read_buffer));
