...
namespace write_read_test
{
    ...
    TEST_P(WriteReadTest, WRITEREADWRITETest)
    {
        ...
        for (unsigned int p = 0; p < ssd_config->get_total_pages_ns(ID_SECTOR_NS0); p++)
        {
            FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS0, p * ssd_config->get_page_size(), 1, NULL);
        ...
        for (unsigned int p = 0; p < ssd_config->get_total_pages_ns(ID_SECTOR_NS1); p++)
        {
            FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS1, p * ssd_config->get_page_size(), 1, NULL);
        ...
        for (uint32_t nsid = 0; nsid < ssd_config->get_num_namespaces(); nsid++)
        {
            ...
            if (totalPages > 0 && ssd_config->get_ns_strategy(nsid) == FTL_NS_SECTOR)
            {
                ...
                for (size_t p = 0; p < totalPages; p++)
                {
                    ...
                    ASSERT_EQ(FTL_SUCCESS, FTL_WRITE_SECT(g_device_index, nsid, p * pageSize, 1, write_buffer));
                }

                for (size_t p = 0; p < totalPages; p++)
                {
                    memset(write_buffer, (uint8_t)(p % 255), pageSize);
                    ASSERT_EQ(FTL_SUCCESS, FTL_READ_SECT(g_device_index, nsid, p * pageSize, 1, read_buffer));
    ...
    TEST_P(WriteReadTest, BoundaryViolationTest)
    {
        ...
        ASSERT_EQ(FTL_FAILURE, FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS0, invalid_offset, 1, buffer));
        ASSERT_EQ(FTL_FAILURE, FTL_READ_SECT(g_device_index, ID_SECTOR_NS0, invalid_offset, 1, buffer));
    ...
    void ns_stress_worker(int device_idx, uint32_t nsid, size_t total_pages, size_t page_size)
    {
        ...
        for (int loop = 0; loop < 3; loop++)
        {
            for (size_t p = 0; p < total_pages; p++)
            {
                ...
                ASSERT_EQ(FTL_SUCCESS, FTL_WRITE_SECT(device_idx, nsid, p * page_size, 1, write_buffer));
            }

            for (size_t p = 0; p < total_pages; p++)
            {
                memset(write_buffer, (uint8_t)((p + nsid + loop) % 255), page_size);
                ASSERT_EQ(FTL_SUCCESS, FTL_READ_SECT(device_idx, nsid, p * page_size, 1, read_buffer));
