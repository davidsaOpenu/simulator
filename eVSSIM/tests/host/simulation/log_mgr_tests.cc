...
//during the garbage collection test nothing should be read as the struct is empty
#define READ_GC_LENGTH 0
...
namespace log_mgr_tests {
    ...
    TEST_P(LogMgrUnitTest, NormalStringWriteRead) {
        ...
        ASSERT_EQ(sizeof(TEST_STRING), logger_read(_logger, (Byte*) res, sizeof(TEST_STRING),RT_ANALYZER));
    ...
    TEST_P(LogMgrUnitTest, ReadBeforeWriteOnLogBoundry) {
        ...
        for (unsigned int i = 0; i < ((LOG_SIZE)-1); i++)
            ASSERT_EQ(1, logger_read(_logger, (Byte *)res, READ_LENGTH, RT_ANALYZER));
        ASSERT_EQ(0, logger_read(_logger, (Byte *)res, length ,RT_ANALYZER));
    }
    /**
    ...
    TEST_P(LogMgrUnitTest, CleanRTAnalyzerNotDone)
    {
        ...
        for (unsigned long int i = 0; i < ((LOG_SIZE * _logger->number_of_allocated_logs)); i++) {
            ASSERT_EQ(0, logger_write(_logger, &placeholder, RT_ANALYZER));
        }
    ...
    TEST_P(LogMgrUnitTest, CleanOfflineAnalyzerNotDone)
    {
        ...
        // read full log
        for (unsigned long int i = 0; i < (LOG_SIZE * _logger->number_of_allocated_logs); i++) {
            ASSERT_EQ(1, logger_read(_logger, (Byte*)read_res, READ_LENGTH, RT_ANALYZER));
        }
    ...
    TEST_P(LogMgrUnitTest, CleanHalfOfTheLogger)
    {
        ...
        for(uint32_t i = 0; (i < (_logger->number_of_allocated_logs/2)) && (log != _logger->dummy_log); i++) {
            log->analyzer_done[OFFLINE_ANALYZER] = true;
            log = log->next;
        }
    ...
    TEST_P(LogMgrUnitTest, FullBuffer) {
        ...
        ASSERT_EQ(sizeof(offset), logger_read(_logger, offset, sizeof(offset),RT_ANALYZER));
    ...
    TEST_P(LogMgrUnitTest, FullPoolStringWriteRead) {
        ...
        // read full log
        for (unsigned long int i = 0; i < (LOG_SIZE * _logger->number_of_allocated_logs); i++) {
            ASSERT_EQ(1, logger_read(_logger, &placeholder, READ_LENGTH, RT_ANALYZER));
        }
    ...
    TEST_P(LogMgrUnitTest, SuccessiveWrites) {
        ...
        Byte first_res;
        Byte second_res;
        ASSERT_EQ(0, logger_write(_logger, &first, RT_ANALYZER));
        ASSERT_EQ(0, logger_write(_logger, &second, RT_ANALYZER));
        ASSERT_EQ(1, logger_read(_logger, &first_res, READ_LENGTH, RT_ANALYZER));
        ASSERT_EQ(1, logger_read(_logger, &second_res, READ_LENGTH, RT_ANALYZER));
    ...
    TEST_P(LogMgrUnitTest, ReadBeforeWrite) {
        Byte placeholder = 'J';
        Byte res;
        ASSERT_EQ(0, logger_read(_logger, &res, READ_LENGTH, RT_ANALYZER));
        ASSERT_EQ(0, logger_write(_logger, &placeholder, RT_ANALYZER));
        ASSERT_EQ(1, logger_read(_logger, &res, READ_LENGTH, RT_ANALYZER));
    ...
    TEST_P(LogMgrUnitTest, PhysicalCellRead) {
        ...
        NEXT_PHYSICAL_CELL_READ_LOG(_logger, &res, RT_ANALYZER);
    ...
    TEST_P(LogMgrUnitTest, PhysicalCellProgram) {
        ...
        NEXT_PHYSICAL_CELL_PROGRAM_LOG(_logger, &res, RT_ANALYZER);
        ASSERT_EQ(log.channel, res.channel);
        ASSERT_EQ(log.block, res.block);
        ASSERT_EQ(log.page, res.page);
    }
    ...
    TEST_P(LogMgrUnitTest, LogicalCellProgram) {
        ...
        NEXT_LOGICAL_CELL_PROGRAM_LOG(_logger, &res, RT_ANALYZER);
        ASSERT_EQ(log.channel, res.channel);
        ASSERT_EQ(log.block, res.block);
        ASSERT_EQ(log.page, res.page);
    }
    ...
    TEST_P(LogMgrUnitTest, GarbageCollection) {
        ...
        NEXT_GARBAGE_COLLECTION_LOG(_logger, &res, RT_ANALYZER);
        ASSERT_EQ(0, logger_read(_logger, &placeholder, READ_GC_LENGTH, RT_ANALYZER));
    }
    /**
    ...
    TEST_P(LogMgrUnitTest, RegisterReadLog) {
        ...
        RegisterReadLog res;
        NEXT_REGISTER_READ_LOG(_logger, &res, RT_ANALYZER);
        ASSERT_EQ(log.channel, res.channel);
    ...
    TEST_P(LogMgrUnitTest, RegisterWriteLog) {
        ...
        NEXT_REGISTER_WRITE_LOG(_logger, &res, RT_ANALYZER);
        ASSERT_EQ(log.channel, res.channel);
        ASSERT_EQ(log.die, res.die);
        ASSERT_EQ(log.reg, res.reg);
    }
    ...
    TEST_P(LogMgrUnitTest, BlockEraseLog) {
        ...
        BlockEraseLog res;
        NEXT_BLOCK_ERASE_LOG(_logger, &res, RT_ANALYZER);
        ASSERT_EQ(log.channel, res.channel);
        ASSERT_EQ(log.die, res.die);
        ASSERT_EQ(log.block, res.block);
    ...
    TEST_P(LogMgrUnitTest, ChannelSwitchToReadLog) {
        ...
        NEXT_CHANNEL_SWITCH_TO_READ_LOG(_logger, &res, RT_ANALYZER);
        ASSERT_EQ(log.channel, res.channel);
    }
    /**
    ...
    TEST_P(LogMgrUnitTest, ChannelSwitchToWriteLog) {
        ...
        NEXT_CHANNEL_SWITCH_TO_WRITE_LOG(_logger, &res, RT_ANALYZER);
        ASSERT_EQ(log.channel, res.channel);
    }
    ...
    /*TEST_P(LogMgrUnitTest, BasicOfflineAnalyzer) {
        ...
        // read full log
        for (unsigned long int i = 0; i < (LOG_SIZE * _logger->number_of_allocated_logs); i++) {
            ASSERT_EQ(1, rt_logger_read(_logger, (Byte*)read_res, 1));
        }

        // free && clean up's of the offline log analyzer
        __atomic_store_n(&analyzer->exit_loop_flag, 1, __ATOMIC_RELEASE);
