...
namespace ssd_io_emulator_tests {
    ...
    TEST_P(SSDIoEmulatorUnitTest, WriteRead2) {
        ...
        size_t block_x_flash = ssd_config->get_pages_ns(ID_SECTOR_NS0) / ssd_config->get_pages_per_block();
    ...
    TEST_P(SSDIoEmulatorUnitTest, WriteAmplificationTest) {
        ...
        const size_t page_x_flash = (ssd_config->get_pages_ns(ID_SECTOR_NS0) + ssd_config->get_pages_ns(ID_SECTOR_NS1));

        // Write all flash.
        for(int x=0; x<2; x++){
            for(size_t p=0; p < ssd_config->get_pages_ns(ID_SECTOR_NS0); p++){
                ASSERT_EQ(FTL_SUCCESS, _FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS0, p * ssd_config->get_page_size(), 1, NULL));
            }

            for(size_t p=0; p < ssd_config->get_pages_ns(ID_SECTOR_NS1); p++){
                ASSERT_EQ(FTL_SUCCESS, _FTL_WRITE_SECT(g_device_index, ID_SECTOR_NS1, p * ssd_config->get_page_size(), 1, NULL));
            }
