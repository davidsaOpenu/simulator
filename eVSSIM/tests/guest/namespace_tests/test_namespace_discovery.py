...
class TestNamespaceDiscovery(unittest.TestCase):
    ...
    def test_07_mixed_strategies_present(self):
        """Verify namespace strategies match the runtime storage strategy."""
        runtime_strategy = int(os.environ.get("EVSSIM_RUNTIME_STORAGE_STRATEGY", str(SECTOR)))
        print("\n[TEST] Verifying namespace strategies (runtime=%d)..." % runtime_strategy)
        ...
        if runtime_strategy == SECTOR:
            self.assertGreater(len(found_sector), 0,
                               "Strategy=1: should have at least 1 sector namespace")
            self.assertEqual(len(object_devs), 0,
                             "Strategy=1: all namespaces should be sector")
        else:
            self.assertGreater(len(found_object), 0,
                               "Strategy=2: should have at least 1 object namespace")
            self.assertEqual(len(sector_devs), 0,
                             "Strategy=2: all namespaces should be object")

    def _check_disk_namespace_count(self, disk, expected_count, found_disk_ns):
        """Return an error string if disk's namespace count doesn't match, else None."""
        ...
        if actual_count != expected_count:
            return "nvme%d: expected %d namespaces, found %d" % (
    ...
    def test_08_multi_namespace_disks(self):
        ...
        for disk in sorted(expected_disk_ns.keys()):
            err = self._check_disk_namespace_count(
                disk, expected_disk_ns[disk], found_disk_ns)
            if err:
                errors.append(err)
