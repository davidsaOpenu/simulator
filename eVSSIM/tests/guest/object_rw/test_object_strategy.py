...
Usage:
    ...
    NVME_OBJECT_DEVICE=/dev/nvme0n1 sudo nosetests -v object_rw/test_object_strategy.py
"""
...
class NvmeObjectDevice(object):
    ...
    def exists(self, obj_path):
        """Check object existence by looking up its 8-byte key in the nvme-cli
        SQLite database and passing the resulting 32-char hex string to obje.

        nvme-cli's obje expects a hex-encoded 128-bit key, not a file path.
        The key is stored in keys.db (relative to nvme_cli_dir) as a BLOB by
        objw.  After objd the entry is removed, so a missing row means the
        object was deleted.
        """
        db_path = os.path.join(self.nvme_cli_dir, 'keys.db')
        try:
            conn = sqlite3.connect(db_path)
            row = conn.execute(
                'SELECT key FROM MappingTable WHERE path = ?', (obj_path,)
            ).fetchone()
            conn.close()
        except Exception:
            return False
        ...
        if row is None:
            # Key was removed from the database (object was deleted).
            return False

        # The key blob is 8 bytes stored as a little-endian uint64.
        # Unpack directly; struct accepts str/buffer (Python 2.7) and bytes (3).
        key_lo, = struct.unpack('<Q', row[0])

        # obje parses a hex string: first 16 hex chars -> key_low,
        # last 16 hex chars -> key_high.  key_high is always 0 for keys
        # generated by objw (Jenkins hash fits in 64 bits).
        hex_key = '%016x%016x' % (key_lo, 0)

        try:
            self._run(['./nvme', 'obje', self.device, hex_key])
            return True
...
class TestObjectReadWrite(unittest.TestCase):
    ...
    def test_03_overwrite_object(self):
        """Overwrite an existing object; store must succeed and return the new content.

        The KV store allows overwriting a key (NVME_SUCCESS). The capacity limit
        is not enforced for updates to existing keys. Verified by reading back
        and comparing against the new content, not the original.
        """
