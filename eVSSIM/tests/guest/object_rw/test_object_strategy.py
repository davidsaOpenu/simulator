...
class TestObjectReadWrite(unittest.TestCase):
    ...
    def test_03_overwrite_object(self):
        """Overwrite an object with new content, verify new data is returned."""
...
class TestObjectDelete(unittest.TestCase):
    ...
    def test_01_delete_object(self):
        """Write an object, delete it, verify it no longer exists."""
...
class TestObjectCapacity(unittest.TestCase):
    ...
    def test_01_fill_to_capacity_and_overflow(self):
        ...
        print("  Created %d objects" % len(self._cleanup_list))
        self.assertGreater(len(self._cleanup_list), 0,
                           "Should create at least 1 object")

        # Try overflow — must be rejected
        ...
        try:
            ...
            self._cleanup_list.append(overflow_path)
            print("  WARNING: Overflow object was accepted")
        except subprocess.CalledProcessError:
            overflow_failed = True
            print("  OK: Overflow correctly rejected")
        finally:
            if os.path.exists(overflow_path) and \
               overflow_path not in self._cleanup_list:
                os.remove(overflow_path)

        self.assertTrue(overflow_failed,
                        "Overflow object should have been rejected "
                        "but was accepted")

        print("  OK: Capacity test complete")
