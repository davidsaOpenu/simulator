...
def _find_ssd_conf():
    """Locate ssd.conf or ssd.conf.template at runtime.

    Search order:
    1. SSD_CONF_PATH environment variable override.
    2. ssd.conf in the same directory as this file (runtime-generated).
    3. ssd.conf.template in the builder docker directory (source tree).
    """
    ...
    if os.path.isfile(local):
        return local
    # Fall back to ssd.conf.template in the builder directory.
    # device_map.py lives at simulator/eVSSIM/tests/guest/ so the template
    # is three levels up then into infra/builder/docker/.
    template = os.path.normpath(
        os.path.join(here, "..", "..", "..", "infra", "builder", "docker",
                     "ssd.conf.template"))
            ...
            if len(parts) == 2:
                ...
                if key == "STORAGE_STRATEGY":
                    try:
                        ...
                        # Template variable (e.g. %(EVSSIM_RUNTIME_STORAGE_STRATEGY)s)
                        # not yet substituted; default to sector at parse time.
                        ns_props["STORAGE_STRATEGY"] = SECTOR
