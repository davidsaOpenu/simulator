...
class test_NvmeCli(object):
    ...
    def test_list(self):
        ...
        lines = self.device.objl().split('\n')
        for oid in xrange(0, count):
        	line = lines[oid].split(' ')
        	print("objl is:" + str(line[0]) + " " + str(line[1]) + " real is:" + str(objects[oid][0]) + " " + str(objects[oid][1]))
        	assert line[0] == objects[oid][0] and line[1] == objects[oid][1]
