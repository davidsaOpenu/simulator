...
namespaces_test() {
    ...
    set +e
    evssim_guest "cd ./guest; mkdir Logs; sudo EVSSIM_RUNTIME_STORAGE_STRATEGY=2 VSSIM_NEXTGEN_BUILD_SYSTEM=1 nosetests -v --with-xunit --xunit-file=guest_tests_results.xml namespace_tests"
    test_rc=$?
    set -e
