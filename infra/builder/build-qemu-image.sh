...
if [ "$#" -ne 2 ]; then
    ...
    echo "Builds a guest VM image based on Ubuntu VM_VERSION version, building it from inside a host docker container based on Ubuntu CONTAINER_VERSION version."
    exit 2
fi
vm_version="$1"
docker_tag="$2"
