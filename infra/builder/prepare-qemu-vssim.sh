...
# QEMU 11's hw/vssim/meson.build compiles simulator sources through the
# gitignored hw/vssim/simulator view.  compile-qemu.sh runs git clean -fdx
# before every build, so that view must be recreated from the authoritative
# eVSSIM tree after the clean and before QEMU configuration.  Keep this setup
# here rather than duplicating simulator sources in the QEMU repository.
