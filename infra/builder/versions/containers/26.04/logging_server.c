...
static const struct lws_http_mount http_mount = {
    .mount_next = NULL,                 // pointer to the next item in the linked list (None here)
    .mountpoint = "/",                  // mount point in URL name space
    .origin = WWW_DIR,                  // path to be mounted on
    .def = "index.html",                // default target
    .protocol = NULL,
    .cgienv = NULL,
...
//    .append_path = 0,
//    .no_ws_upgrades = 0,
    .origin_protocol = LWSMPRO_FILE,    // mount file system
    .mountpoint_len = 1,                // strlen("/")
...
void log_server_update(SSDStatistics stats, void *uid) {
    ...
    if (uid == NULL) {}/* do nothing to satisfy warning */
}
