...
# and the guest and EXOFS logs are still shipped. The follow-up change
# "ELK host-test metrics: per-case gating" replaces this script and keeps only
# the logs of the runs its ELK checks read.
LOGS_TMP="$(mktemp -d)"
