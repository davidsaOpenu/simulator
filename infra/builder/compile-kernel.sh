...
TARGET_KERNEL_HASH=$(evssim_run_at_folder "$version" $EVSSIM_KERNEL_FOLDER "git rev-parse $EVSSIM_KERNEL_BRANCH")

if [[ "$CURRENT_KERNEL_HASH" != "$TARGET_KERNEL_HASH" ]]; then
	GIT_CLEAN_RESET="git clean -fdx && git reset --hard HEAD"
    evssim_run_at_folder "$version" $EVSSIM_KERNEL_FOLDER $GIT_CLEAN_RESET
	evssim_run_at_folder "$version" $EVSSIM_KERNEL_FOLDER "git checkout $EVSSIM_KERNEL_BRANCH"
	evssim_run_at_folder "$version" $EVSSIM_KERNEL_FOLDER $GIT_CLEAN_RESET
else
