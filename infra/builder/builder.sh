...
evssim_qemu () {
    ...
    if [[ "$EVSSIM_QEMU_SIMULATOR_ENABLED" =~ y.* ]]; then
        ...
        for device_size in "${device_sizes[@]}"; do
            ...
            drive_args="$drive_args -drive format=vssim,size=$device_size,simulator=$device_simulator,if=none,id=memory$device_index"

            # Create NVMe device argument
            device_args="$device_args -device nvme,drive=memory$device_index,serial=$serial_number"
