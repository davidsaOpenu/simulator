...
evssim_run_at_path () {
    local path=$1
    local args="${@:2}"
    docker run --rm -i $docker_extra_tty $EVSSIM_DOCKER_XOPTIONS -p 2003:2003 --privileged --env-file <(evssim_all_env) -v $EVSSIM_ROOT_PATH:$EVSSIM_DOCKER_ROOT_PATH $EVSSIM_DOCKER_IMAGE_NAME bash -c "cd $path; $args"
}
