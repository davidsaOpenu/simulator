...
evssim_use_host_network() {
    local network_mode=${EVSSIM_DOCKER_NETWORK_MODE:-auto}
...
evssim_qemu_flush_disk () {
    ...
    local ssh_key=$EVSSIM_ROOT_PATH/$EVSSIM_BUILDER_FOLDER/docker/id_rsa

    set +e
    timeout ${flush_timeout}s \
        ssh -q -T -i $ssh_key -p $EVSSIM_QEMU_PORT \
            -o ConnectTimeout=5 \
...
evssim_wait_for_guest () {
    ...
    while true; do
        if ssh -q -i $ssh_key -p $EVSSIM_QEMU_PORT \
            -o ConnectTimeout=2 \
            -o ConnectionAttempts=1 \
            -o UserKnownHostsFile=/dev/null \
            -o StrictHostKeyChecking=no \
