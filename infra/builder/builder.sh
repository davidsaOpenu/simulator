...
evssim_guest_ssh_key_path() {
    printf '%s\n' "$EVSSIM_ROOT_PATH/$EVSSIM_BUILDER_FOLDER/docker/id_rsa"
}
...
evssim_guest_ssh_args() {
    ...
    ssh_key="$(evssim_guest_ssh_key_path)"
    ...
    EVSSIM_GUEST_SSH_ARGS+=(-i "$ssh_key" -p "$EVSSIM_QEMU_PORT")
...
chmod 600 "$(evssim_guest_ssh_key_path)"
...
evssim_guest_copy () {
    ...
    ssh_key="$(evssim_guest_ssh_key_path)"
    scp -r -o ConnectionAttempts=1024 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o PasswordAuthentication=no -o PubkeyAcceptedKeyTypes=+ssh-rsa -i "$ssh_key" -P "$EVSSIM_QEMU_PORT" "$EVSSIM_QEMU_UBUNTU_USERNAME@localhost:$docker_file_path" "$output_file_path"
}
