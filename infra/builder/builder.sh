...
evssim_qemu () {
    ...
    if [[ "$EVSSIM_QEMU_SIMULATOR_ENABLED" =~ y.* ]]; then
        ...
        for ((device_index=1; device_index<=$device_count; device_index++)); do
            ...
            for namespace_size in "${namespace_sizes[@]}"; do
                ...
                # All drives are created here; secondary namespaces (nsid>0)
                # are discovered by nvme_realize via DRIVE_ID in ssd.conf.
            ...
            # Create one NVMe device per controller, referencing the primary drive.
            # Secondary namespace drives are resolved from ssd.conf at runtime.
            local ns_prop=""
            if [ $num_ns -gt 1 ]; then
                ns_prop=",num_namespaces=$num_ns"
            fi
            device_args="$device_args -device nvme,drive=$first_drive_id,serial=$serial_number${ns_prop}"
            ((++serial_number))
        done
    ...
    if [[ "$EVSSIM_RUNTIME_ALWAYS_RESET" =~ y.* ]]; then
        find "$EVSSIM_ROOT_PATH/$EVSSIM_DATA_FOLDER" -name "*.dat" -delete
    fi
...
evssim_copy_tools () {
    ...
    # copy ssd.conf template so guest tests can parse device topology dynamically
    evssim_run_mounted "cp $EVSSIM_DOCKER_ROOT_PATH/$EVSSIM_BUILDER_FOLDER/docker/ssd.conf.template guest/ssd.conf"
}
