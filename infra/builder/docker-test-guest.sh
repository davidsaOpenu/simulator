...
guest_test() {
    ...
    set +e
    evssim_guest "cd ./guest; mkdir Logs; sudo VSSIM_NEXTGEN_BUILD_SYSTEM=1 python -m pytest -v --junit-xml=./Logs/results.xml --capture=sys $test_name"
    test_rc=$?
    set -e
...
# Run disk tests
guest_test "$test_directory_base" 1 off nvme_compliance_tests
guest_test "$test_directory_base" 1 off fio_tests
