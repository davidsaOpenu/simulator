...
VALGRIND_CMD="${VALGRIND_CMD:-valgrind --leak-check=full --error-exitcode=2 --suppressions=$EVSSIM_DOCKER_ROOT_PATH/$SIM_DIR/gtest_params.supp}"
...
if [[ "${SKIP_LEAKCHECK:-}" != "1" ]]; then
  ...
  echo "[docker-test-host-elk] leak-check (valgrind minus heavy suites, then native heavy suites) started in background; its output is printed after the metric runs"
  (
    ...
    # heavy suites run natively below: valgrind needs every path, not their volume (WriteReadTest alone: 4h11 valgrind vs ~10min native)
    evssim_run_at_folder "$version" "$SIM_DIR" "$VALGRIND_CMD ./simulation_tests_main --ci --gtest_filter=*-*WriteReadTest*:*OnfiCommandsTest*"
    evssim_run_at_folder "$version" "$SIM_DIR" "./simulation_tests_main --ci --ssd_write_read_test"
    evssim_run_at_folder "$version" "$SIM_DIR" "./simulation_tests_main --ci --onfi_ops_test"
    evssim_run_at_folder "$version" "$UNIT_DIR" "$VALGRIND_CMD ./unit_tests_main --ci"
  ) > "$LEAK_LOG" 2>&1 &
  LEAK_PID=$!
fi
...
run_group() {
  local flag="$1" out
  out="$(evssim_run_at_folder "$version" "$SIM_DIR" "./simulation_tests_main $flag" 2>&1)" || {
...
if [[ -n "$LEAK_PID" ]]; then
  ...
  LEAK_PID=""
  echo "================================================================"
  echo "[docker-test-host-elk] leak-check + native heavy suites output:"
  cat "$LEAK_LOG"
  if (( leak_rc )); then
