...
if [[ "${SKIP_LEAKCHECK:-}" != "1" ]]; then
  ...
  (
    ...
    evssim_run_at_folder "$SIM_DIR" "$VALGRIND_CMD ./simulation_tests_main --ci --gtest_filter=*-*WriteReadTest*:*OnfiCommandsTest*"
    evssim_run_at_folder "$SIM_DIR" "./simulation_tests_main --ci --ssd_write_read_test"
    evssim_run_at_folder "$SIM_DIR" "./simulation_tests_main --ci --onfi_ops_test"
