...
detect_container_runtime() {
  if command -v podman >/dev/null 2>&1; then
    ...
    # Use Podman-native compose; do NOT fall back to raw docker-compose here  
    if command -v podman-compose >/dev/null 2>&1; then
      COMPOSE_CMD="podman-compose"
    else
      echo "Error: Podman is installed but no compose frontend found (podman compose / podman-compose)"
      exit 1
    fi
    COMPOSE_EXEC_T_OPT=""
