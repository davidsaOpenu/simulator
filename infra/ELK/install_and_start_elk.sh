...
ELK_NO_PROXY_HOSTS="localhost,127.0.0.1,127.0.1.1,local.home,docker.elastic.co,docker-auth.elastic.co,elastic.co"

# Prepend our hosts to any existing NO_PROXY / no_proxy
if [[ -n "${NO_PROXY:-}" ]]; then
  export NO_PROXY="${ELK_NO_PROXY_HOSTS},${NO_PROXY}"
elif [[ -n "${no_proxy:-}" ]]; then
  export NO_PROXY="${ELK_NO_PROXY_HOSTS},${no_proxy}"
else
  export NO_PROXY="${ELK_NO_PROXY_HOSTS}"
fi

export no_proxy="$NO_PROXY"

echo "Using NO_PROXY for this run: $NO_PROXY"

ensure_podman_socket() {
  local uid
  uid="$(id -u)"
  local user_sock="/run/user/${uid}/podman/podman.sock"
  local user_uri="unix://$user_sock"

  # 1) Try to use systemd-managed *user* podman.socket (local dev / desktop)
  if command -v systemctl >/dev/null 2>&1; then
    # If it's already active, just use it
    if systemctl --user is-active --quiet podman.socket 2>/dev/null; then
      export DOCKER_HOST="$user_uri"
      echo "Using active user podman.socket at $user_uri"
      return 0
    fi

    echo "Trying to enable/start podman.socket via systemd --user..."
    if systemctl --user enable --now podman.socket >/dev/null 2>&1; then
      # Give systemd a moment to create the socket and activate the service
      sleep 1
      if systemctl --user is-active --quiet podman.socket 2>/dev/null; then
        export DOCKER_HOST="$user_uri"
        echo "Using systemd-managed user podman.socket at $user_uri"
        return 0
      else
        echo "podman.socket is not active even after systemctl --user enable --now"
      fi
    else
      echo "systemctl --user failed (no user bus / no unit?) – falling back to private podman service."
    fi
  fi

  # 2) Headless / CI fallback: start our own rootless podman API service.
  local fallback_sock="/tmp/podman-${uid}.sock"
  local fallback_uri="unix://$fallback_sock"

  echo "Starting private podman system service on $fallback_uri (no usable user systemd)..."
  rm -f "$fallback_sock" || true
  mkdir -p "$(dirname "$fallback_sock")" || true

  # Start podman system service with NO_PROXY but without HTTP(S)_PROXY,
  # so it never sends registry traffic through the broken proxy.
  env -u HTTP_PROXY -u http_proxy -u HTTPS_PROXY -u https_proxy \
      NO_PROXY="$NO_PROXY" no_proxy="$NO_PROXY" \
      podman system service --time=0 "$fallback_uri" >/tmp/podman-system-service.log 2>&1 &
  local pid=$!

  PODMAN_SYSTEM_SERVICE_PID="$pid"

  # Wait briefly for the socket to appear
  for i in {1..10}; do
    if [ -S "$fallback_sock" ]; then
      export DOCKER_HOST="$fallback_uri"
      echo "podman system service started on $fallback_uri (pid $pid)"
      return 0
    fi
    sleep 1
  done

  echo "Error: podman system service did not become ready on $fallback_uri"
  return 1
}

ensure_docker_socket() {
  # Prefer docker.service; fall back to docker.socket if that's how the distro is set up
  local unit=""
  ...
  if systemctl list-unit-files | grep -q '^docker\.service'; then
    ...
    echo "Error: neither docker.service nor docker.socket systemd unit found"
    exit 1
  fi

  if ! systemctl is-enabled --quiet "$unit" 2>/dev/null; then
    echo "$unit is not enabled – enabling..."
    if ! systemctl enable --now "$unit"; then
      echo "Error: failed to enable/start $unit (do you need sudo?)"
      exit 1
    fi
  fi

  if ! systemctl is-active --quiet "$unit" 2>/dev/null; then
    echo "Error: $unit is not active even after enabling"
    exit 1
  fi
}

detect_container_runtime() {
  # First preference: Podman + podman-compose
  if command -v podman >/dev/null 2>&1 && command -v podman-compose >/dev/null 2>&1; then
    echo "Podman + podman-compose detected; using Podman stack"
    CONTAINER_CMD="podman"
    COMPOSE_CMD="podman-compose"
    COMPOSE_EXEC_T_OPT=""

    if ! ensure_podman_socket; then
      echo "Error: failed to set up Podman API socket"
      exit 1
    fi

  # Second preference: Docker + compose frontend
    ...
    if docker compose version >/dev/null 2>&1; then
      ...
      echo "Error: Docker is installed but no compose frontend found ('docker compose' or 'docker-compose')"
      exit 1
    fi
    ...
    echo "Error: neither a full Podman+podman-compose nor Docker+compose stack was found"
    exit 1
  fi
...
# Ensure tls/entrypoint.sh has the chmod line at the end (world-writable)
# This part is for the CI to be able to delete the files
TLS_ENTRYPOINT="tls/entrypoint.sh"

if [[ ! -f "$TLS_ENTRYPOINT" ]]; then
  echo "[!] Warning: $TLS_ENTRYPOINT not found, cannot patch TLS entrypoint" >&2
  exit 0
fi

last_line=$(tail -n 1 "$TLS_ENTRYPOINT" 2>/dev/null)
target_line='chmod -R 0777 tls/certs || echo "[!] Warning: failed to chmod tls/certs" >&2'

if [[ "$last_line" == "$target_line" ]]; then
  echo "[*] chmod line already present at end of $TLS_ENTRYPOINT, skipping patch"
else
  echo "[*] Patching $TLS_ENTRYPOINT to add chmod block at EOF"
  cat >> "$TLS_ENTRYPOINT" <<'EOF'

chmod -R 0777 tls/certs || echo "[!] Warning: failed to chmod tls/certs" >&2
EOF
fi
