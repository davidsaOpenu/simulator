...
setup_elasticsearch_passwords() {
  ...
  for i in $(seq 1 12); do
    if curl -k -s --connect-timeout 5 https://elasticsearch:9200 >/dev/null; then
      break
    fi
  ...
  curl -k -s -f -u "elastic:$ELASTIC_PASSWORD" \
       https://elasticsearch:9200/_security/_authenticate >/dev/null && elastic_ok=true

  kib_ok=false
  curl -k -s -f -u "kibana_system:$KIBANA_SYSTEM_PASSWORD" \
       https://elasticsearch:9200/_security/_authenticate >/dev/null && kib_ok=true
  ...
  if ! $elastic_ok; then
    ...
    do
      ...
      curl -k -s -f -u "elastic:$ELASTIC_PASSWORD" \
           ...
           -X POST "https://elasticsearch:9200/_security/user/$username/_password" \
  ...
  if curl -k -s -f -u "kibana_system:$KIBANA_SYSTEM_PASSWORD" \
        https://elasticsearch:9200/_security/_authenticate >/dev/null; then
...
check_elasticsearch_health() {
    ...
    for attempt in $(seq 1 30); do
        ...
        health_response=$(curl -k -s -f --connect-timeout 5 -u "elastic:$ELASTIC_PASSWORD" "https://elasticsearch:9200/_cluster/health" 2>/dev/null) && {
            cluster_status=$(echo "$health_response" | grep -o '"status":"[^"]*"' | cut -d':' -f2 | tr -d '"')
            echo "Elasticsearch is up and running on HTTPS!"
            echo "Cluster status: $cluster_status"
            echo "Access Elasticsearch at: https://elasticsearch:9200"
            return 0
        }
...
echo "  Elasticsearch: https://elasticsearch:9200"
