...
ensure_podman_socket() {
  # Check if podman.socket is enabled; if not, try to enable+start it
  if ! systemctl --user is-enabled --quiet podman.socket 2>/dev/null; then
    echo "podman.socket is not enabled for this user – enabling..."
    if ! systemctl --user enable --now podman.socket; then
      echo "Error: failed to enable/start podman.socket"
      exit 1
    fi
  fi
  ...
  if ! systemctl --user is-active --quiet podman.socket 2>/dev/null; then
    echo "Error: podman.socket is not active even after enabling"
    exit 1
  fi
}
