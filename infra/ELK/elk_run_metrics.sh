...
_json_get() {
  local path="$1" default="${2-}" raw keys_json
  raw="$(cat)"
  if ! jq -e . >/dev/null 2>&1 <<<"$raw"; then
    echo "[elk_run_metrics] _json_get: invalid JSON for '$path': $(head -c 200 <<<"$raw")" >&2
    return 1
  fi
  if jq -e 'has("error")' >/dev/null 2>&1 <<<"$raw"; then
    echo "[elk_run_metrics] _json_get: ES error for '$path': $(jq -c '.error.type // .error' <<<"$raw")" >&2
    return 1
  fi
  keys_json="[]"
  [[ -n "$path" ]] && keys_json="$(jq -Rc 'split(".")' <<<"$path")"
  jq -r --arg d "$default" --argjson keys "$keys_json" \
    '(try getpath($keys) catch null) as $v | if $v == null then $d else $v end' <<<"$raw"
}
...
# Shards can still be initializing right after elk_wipe deletes the data stream and
# filebeat recreates it; that window answers _count with search_phase_execution_exception.
# Retrying here covers both callers (elk_wipe and elk_settle).
elk_doc_count() {
  local attempt c
  for attempt in 1 2 3 4 5; do
    if c="$(_es "$INDEX_PATTERN/_count" | _json_get count 0)"; then
      printf '%s' "$c"
      return 0
    fi
    sleep 2
...
elk_wipe() {
  ...
  while (( SECONDS < deadline )); do
    if ! c="$(elk_doc_count)"; then
      echo "[elk_run_metrics] FAIL: cannot read doc count during wipe (ES unreachable?)" >&2
      return 1
    fi
    if [[ "$c" != "0" ]]; then
      _es "_data_stream/$INDEX_PATTERN" -X DELETE >/dev/null
...
elk_settle() {
  ...
  while (( SECONDS < deadline )); do
    if ! c="$(elk_doc_count)"; then
      echo "[elk_run_metrics] FAIL: cannot read doc count while settling (ES unreachable?)" >&2
      return 1
    fi
    if (( c > 0 && c == prev )); then
      stable=$((stable + 1))
