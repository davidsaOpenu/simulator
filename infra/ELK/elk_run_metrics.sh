#!/bin/bash
# =============================================================================
# elk_run_metrics.sh  —  run.id-scoped ELK metric extraction + reconciliation
#
...
# every event) instead of a fragile time window. This gives exact,
# order-independent isolation (see docs/adr/0002, 0003).
#
# Public functions:
#   elk_reconcile   RUN_ID EXPECTED_COUNT   -> 0 when ES holds exactly EXPECTED, else 1
#   elk_query_metrics RUN_ID                -> prints "KEY=VALUE" lines for the 9 metrics
#
# Env knobs: ES_URL, INDEX_PATTERN, ELASTIC_PASSWORD, RECONCILE_TIMEOUT_SECS,
#            RECONCILE_INTERVAL_SECS.
# =============================================================================

ES_URL="${ES_URL:-https://localhost:9200}"
INDEX_PATTERN="${INDEX_PATTERN:-filebeat-*}"
RECONCILE_TIMEOUT_SECS="${RECONCILE_TIMEOUT_SECS:-180}"
RECONCILE_INTERVAL_SECS="${RECONCILE_INTERVAL_SECS:-3}"
...
# -----------------------------------------------------------------------------
# Reconciliation: wait until ES holds EXACTLY the producer's emitted count for
# this run.id (proof of ingestion completeness, not a timing heuristic). A
# healthy pipeline always converges; a genuinely broken one times out into a
# hard, diagnosable failure. Overshoot (> expected) is flagged immediately as
# probable duplicate ingestion (should be impossible once filebeat maps _id from
# run.id+seq, but we detect it rather than silently passing).
# -----------------------------------------------------------------------------
elk_reconcile() {
  local run_id="$1" expected="$2"
  local deadline=$((SECONDS + RECONCILE_TIMEOUT_SECS)) c prev=-1 stable=0
  echo "[elk_run_metrics] reconciling run.id=$run_id against manifest=$expected (timeout ${RECONCILE_TIMEOUT_SECS}s)"

  while (( SECONDS < deadline )); do
    c="$(elk_run_count "$run_id")"
    if (( c > expected )); then
      echo "[elk_run_metrics] FAIL: ES has $c docs > manifest $expected for run.id=$run_id (probable duplicate ingestion; check filebeat _id mapping)"
      return 1
    fi
    if (( c == expected )); then
      echo "[elk_run_metrics] reconciled: ES holds all $expected docs for run.id=$run_id"
      return 0
    fi
  ...
  # Timed out: hard failure with diagnostics (never assert on partial data).
  echo "[elk_run_metrics] FAIL: timed out reconciling run.id=$run_id — ES=$prev manifest=$expected (short by $((expected - prev)))"
  echo "[elk_run_metrics] cluster health: $(_es _cluster/health | jq -c '{status,number_of_nodes,active_shards}' 2>/dev/null)"
  echo "[elk_run_metrics] hint: filebeat may be behind or rejecting docs; inspect filebeat logs and ES _ingest failures"
  return 1
}
...
# -----------------------------------------------------------------------------
# elk_query_metrics RUN_ID -> the nine metrics as KEY=VALUE lines.
#
