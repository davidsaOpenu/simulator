...
_json_get() {
  ...
  if ! jq -e . >/dev/null 2>&1 <<<"$raw"; then
    echo "[elk_run_metrics] _json_get: invalid JSON for '$path', using default '$default': $(head -c 200 <<<"$raw")" >&2
    echo "$default"
    return 0
  fi
  if jq -e 'has("error")' >/dev/null 2>&1 <<<"$raw"; then
    echo "[elk_run_metrics] _json_get: ES error for '$path', using default '$default': $(jq -c '.error.type // .error' <<<"$raw")" >&2
    echo "$default"
    return 0
  fi
  keys_json="[]"
