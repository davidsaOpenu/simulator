...
# python3 (not jq, which the podman test VM lacks) parses ES JSON below; require it up front.
...
# _json_get <dotted.path> [default] : print JSON value from stdin, or default if missing/null.
_json_get() {
  python3 -c '
import sys, json
default = sys.argv[2]
...
elk_run_count() {
  local run_id="$1" body
  body="$(cat <<EOF
{ "query": $(_runid_filter "$run_id") }
EOF
)"
  _es "$INDEX_PATTERN/_count" -d "$body" | _json_get count 0
}
