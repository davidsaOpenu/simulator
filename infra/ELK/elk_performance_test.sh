...
# All events currently carry device_index=0 (TODO in
# MONITOR/SERVER/logging_server.c log_server_init), so disks 1/2 have no data
# yet: SKIP_IF_NO_DATA=1 (default) passes, =0 fails.

check_disk_perf() {
  local disk_idx="$1"
  local disk_min_write_count disk_min_read_count disk_min_iops_write disk_min_iops_read disk_min_wa disk_max_wa
  disk_min_write_count="$(disk_threshold "$disk_idx" MIN_WRITE_COUNT SECTOR_MIN_WRITE_COUNT)"
  disk_min_read_count="$( disk_threshold "$disk_idx" MIN_READ_COUNT  SECTOR_MIN_READ_COUNT)"
  disk_min_iops_write="$( disk_threshold "$disk_idx" MIN_IOPS_WRITE  SECTOR_MIN_IOPS_WRITE)"
  disk_min_iops_read="$(  disk_threshold "$disk_idx" MIN_IOPS_READ   SECTOR_MIN_IOPS_READ)"
...
JSON
  ...
  if (( total_hits == 0 )); then
    if (( SKIP_IF_NO_DATA == 1 )); then
      echo "[elk_performance_test] SKIP: disk $disk_idx has 0 events (device_index not yet in payload; see logging_server.c TODO). SKIP_IF_NO_DATA=1 → pass."
      return
    fi
  ...
  disk_write_span="$(span_secs "$disk_write_count" "$disk_write_first_ts" "$disk_write_last_ts")"
  disk_read_span="$( span_secs "$disk_read_count"  "$disk_read_first_ts"  "$disk_read_last_ts")"
  ...
  # floor (disk_min_read_count defaults to SECTOR_MIN_READ_COUNT=0). Enforce
  # unconditionally so a configured floor isn't silently skipped on a
  # zero-read window; with the default floor of 0 this still passes.
  ...
  # SECTOR_MIN_IOPS_READ=0.0). disk_read_iops is 0 on a zero-read window, so a
  # configured floor fails as intended instead of being skipped.
