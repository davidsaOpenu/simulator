...
int64_t g_channel_times[MAX_CH_NUM];

void init_channel_times(){
    for (int ch_i = 0; ch_i < MAX_CH_NUM; ++ch_i)
    {
        g_channel_times[ch_i] = 1624186092 + ch_i * 10000;   
        //printf("init %d\n", g_channel_times[ch_i]);
    }
}
...
char* json_LogMetadata( char* dest, char const* name, struct LogMetadata const* LogMetadata, int64_t logging_start_time, int64_t logging_end_time) {
    ...
    if ((logging_start_time != NULL) && (logging_end_time != NULL)){
        printf("Metadata %d\n", logging_start_time);
        printf("Metadata %d\n", logging_end_time);
        char* logging_start_time_s = timestamp_to_str(logging_start_time);
        char* logging_end_time_s = timestamp_to_str(logging_end_time);
        dest = json_nstr( dest, "logging_start_time", logging_start_time_s, strlen(logging_start_time_s)); 
        dest = json_nstr( dest, "logging_end_time", logging_end_time_s, strlen(logging_end_time_s) );       
    }
    ...
    */
    ...
    if ((logging_start_time != NULL) && (logging_end_time != NULL)){
        char* logging_start_time_s = timestamp_to_str(logging_start_time);
        dest = json_nstr( dest, "logging_time", logging_start_time_s, strlen(logging_start_time_s)); 
    }
    else if (LogMetadata != NULL){
        char* logging_start_time_s = timestamp_to_str(logging_start_time);

        dest = json_nstr( dest, "logging_time", LogMetadata->logging_start_time, strlen(LogMetadata->logging_start_time)); 
    }
    return dest;
}
...
char* json_PhysicalCellProgramLog( char* dest, struct PhysicalCellProgramLog const* PhysicalCellProgramLog, const int channel,const  int block, const int page, const char* logging_start_time, const char* logging_end_time ) {
    ...
    char* n = "PhysicalCellProgramLog";
    dest = json_nstr( dest, "type", n, strlen(n)); 
    if ((channel != NULL) && (block != NULL) && (page != NULL) && (logging_start_time != NULL) && (logging_end_time != NULL)){
        dest = json_int( dest, "channel", channel );
        dest = json_int( dest, "block", block );  
        dest = json_int( dest, "page", page );
...
char* json_ChannelSwitchToReadLog( char* dest, struct ChannelSwitchToReadLog const* ChannelSwitchToReadLog, const int channel, const char* logging_start_time, const char* logging_end_time ) {
    ...
    //printf("ChannelSwitchToReadLog  %d\n", &ChannelSwitchToReadLog->metadata->logging_start_time);
    //printf("ChannelSwitchToReadLog     %d\n", &ChannelSwitchToReadLog->metadata.logging_end_time);
