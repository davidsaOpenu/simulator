...
#define REG_WRITE_DELAY 82
#define CELL_PROGRAM_DELAY 900
#define REG_READ_DELAY 82
#define CELL_READ_DELAY 50
#define BLOCK_ERASE_DELAY 2000
#define CHANNEL_SWITCH_DELAY_R 16
#define CHANNEL_SWITCH_DELAY_W 33

bool progarmPage(FILE* fptr, int ch_num, int die, int reg, int block,int page){
  if(g_reg_status[ch_num] == State_Read){
    //dump REGISTER_WRITE 
    //printf("progarmPage %d\n", g_channel_times[ch_num]);
    len = json_RegisterWriteLog( buff, NULL, ch_num, die, reg, g_channel_times[ch_num],g_channel_times[ch_num] + REG_WRITE_DELAY);
    g_channel_times[ch_num] += REG_WRITE_DELAY;
    fprintf(fptr, "{%s}\n", buff);
  }
  //dump PHYSICAL_CELL_PROGRAM
  len = json_PhysicalCellProgramLog( buff, NULL, ch_num, block, page, g_channel_times[ch_num],g_channel_times[ch_num] + CELL_PROGRAM_DELAY);
  g_channel_times[ch_num] += CELL_PROGRAM_DELAY;
  fprintf(fptr, "{%s}\n", buff);
  return true;
}
...
bool readPage (FILE* fptr, int ch_num, int block,int page, int die){
  if(g_reg_status[ch_num] == State_Write){
    ...
    //printf("readpage %d\n", g_channel_times[ch_num]);
    len = json_ChannelSwitchToReadLog( buff, NULL, ch_num , g_channel_times[ch_num],g_channel_times[ch_num] + CHANNEL_SWITCH_DELAY_R);
        g_channel_times[ch_num] += CHANNEL_SWITCH_DELAY_R; 
        fprintf(fptr, "{%s}\n", buff);
    }
        
  //dump PHYSICAL_CELL_READ
  len = json_PhysicalCellReadLog( buff, NULL, ch_num, block, page, g_channel_times[ch_num],g_channel_times[ch_num]);
  fprintf(fptr, "{%s}\n", buff);
  return true;
}
...
bool erase_block(FILE* fptr, int ch_num, int die,  int block){
  len = json_BlockEraseLog( buff, NULL, ch_num, die, block, g_channel_times[ch_num],g_channel_times[ch_num] + BLOCK_ERASE_DELAY);
  g_channel_times[ch_num] += BLOCK_ERASE_DELAY; 
  fprintf(fptr, "{%s}\n", buff);
  return true;
}
...
bool logicalWrite(FILE* fptr, int ch_num, int block, int page, int die){
    if(g_reg_status[ch_num] == State_Read){
        ...
        len = json_ChannelSwitchToWriteLog( buff, NULL, ch_num, g_channel_times[ch_num],g_channel_times[ch_num] + CHANNEL_SWITCH_DELAY_W);
        g_channel_times[ch_num] += CHANNEL_SWITCH_DELAY_W;
        fprintf(fptr, "{%s}\n", buff);
    }
    ...
    len = json_LogicalCellProgramLog( buff, NULL, ch_num, block, page, g_channel_times[ch_num],g_channel_times[ch_num] );
    fprintf(fptr, "{%s}\n", buff);
...
bool chSwich(FILE* fptr, int ch_num, int operation){
  if (g_channel_status[ch_num] != operation){
    ...
    if (operation == State_Write){
        len = json_ChannelSwitchToReadLog( buff, NULL, ch_num , g_channel_times[ch_num],g_channel_times[ch_num] );
        g_channel_times[ch_num] += CHANNEL_SWITCH_DELAY_R;
        fprintf(fptr, "{%s}\n", buff);
        }
    }
...
int main (void)
{
    ...
    FILE *fptr;
    fptr = fopen("generated_log.json","wb");
    init_channel_times();
    /*
